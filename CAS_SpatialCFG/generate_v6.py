...
# -*- coding: utf-8 -*-
"""
Cross-Attention Probing + CAS Safe Generation v6: Training-Free Safe Generation

Architecture:
  WHEN (Global CAS): cos(d_prompt, d_target) > tau (same as v4)
  WHERE (Cross-Attention Probe): probe UNet hidden states with target concept K
                                  during eps_prompt forward pass -> spatial mask
  HOW: Same guidance modes as v4 (hybrid recommended)

Key differences from v4:
  - WHERE uses cross-attention maps instead of noise-based spatial CAS
  - Cross-attention is semantically meaningful (token-to-pixel alignment)
  - Zero extra UNet calls for WHERE detection (probes during existing pass)
  - Supports --where_mode: crossattn (v6) or noise (v4 fallback)
"""
...
# =============================================================================
# Spatial CAS: Per-Pixel Concept Alignment (WHERE — noise fallback)
# =============================================================================
...
def apply_guidance(
    ...
    **kwargs,
    ...
    if guide_mode == "anchor_inpaint":
        ...
        # e_final = e_cfg - t_s*M*(e_target - e_null) + a_s*M*(e_anchor - e_null)
        ...
        # e_safe_cfg = e_null + cfg_scale * (d_prompt - proj(d_prompt, d_target))
        # e_final = e_safe_cfg*(1-a*M) + e_anchor_cfg*(a*M)
        ...
        # d_prompt = e_prompt - e_null (prompt direction)
        # d_target = e_target - e_null (nudity direction)
        # d_safe = d_prompt - proj(d_prompt, d_target) (nudity removed)
        # e_safe_cfg = e_null + cfg_scale * d_safe
        # e_final = e_cfg*(1-s*M) + e_safe_cfg*(s*M)
        d_prompt = eps_prompt - eps_null
        d_target = eps_target - eps_null
...
def parse_args():
    p = ArgumentParser(description="Cross-Attention Probing + CAS Safe Generation v6")
    ...
    # WHERE mode
    p.add_argument("--where_mode", type=str, default="crossattn",
                    choices=["crossattn", "noise"],
                    help="Spatial detection: crossattn (v6) or noise (v4 fallback)")
    ...
    p.add_argument("--save_maps", action="store_true",
                    help="Save spatial CAS maps, attention maps, and masks for debugging")
...
def main():
    ...
    where_label = (f"Cross-Attention Probe, resolutions={args.attn_resolutions}, "
                   f"target_words={args.target_words}"
                   if args.where_mode == "crossattn"
                   else f"Spatial CAS (noise), neighborhood={args.neighborhood_size}")

    print(f"\n{'='*70}")
    print(f"Cross-Attention Probing + CAS Safe Generation v6")
    print(f"{'='*70}")
    ...
    # ---- Setup cross-attention probing (v6) ----
    original_processors = None
    probe_store = None
    probe_token_indices = None

    if args.where_mode == "crossattn":
        # Setup attention probing
        probe_store = AttentionProbeStore()

        # Pre-compute K_target for each attn2 layer (one-time, microseconds)
        target_keys = precompute_target_keys(unet, target_embeds, args.attn_resolutions)
        ...
        # Find token indices for target concept
        ...
        for sample_idx in range(args.nsamples):
            ...
            for step_idx, t in enumerate(scheduler.timesteps):
                ...
                if should_guide:
                    ...
                    # =====================================================
                    # Step 3: WHERE — crossattn or noise
                    # =====================================================
                    if args.where_mode == "crossattn":
                        # Cross-attention spatial mask from probe
                        ...
                        # Use compute_soft_mask on the attention map
                        ...
                        # v4 fallback: noise-based spatial CAS
                    ...
                    # =====================================================
                    # Step 4: HOW — Apply guidance (same as v4)
                    # =====================================================
                    ...
                    if args.save_maps and step_idx % 10 == 0:
                        if args.where_mode == "crossattn":
                            ...
                            # Save spatial CAS map (noise mode)
            ...
            if prompt_idx % 10 == 0 or args.debug:
                tqdm.write(
                    ...
                    f"mask_area={stats['avg_mask_area']:.3f} where={args.where_mode}"
                )

    # ---- Cleanup cross-attention probing ----
    if args.where_mode == "crossattn" and original_processors is not None:
    ...
    summary = {
        "method": "Cross-Attention Probing + CAS Safe Generation v6",
        "args": vars(args),
        "overall": {
            "total_images": n,
            "triggered": n_trig,
