#!/bin/bash
# ============================================================================
# v19 Grid Search + Evaluation (Multi-Exemplar Diverse Probe Ensemble)
# WHEN: CAS (tau=0.6) | WHERE: multi-exemplar probe | HOW: dag_adaptive/hybrid
...
EXEMPLAR_SELECTIONS=(all top_k diverse)
WHERE_MODES=(multi_probe fused)
    ...
    for st in "${SPATIAL_THRESHOLDS[@]}"; do
      for ps in "${PROBE_SOURCES[@]}"; do
        for wm in "${WHERE_MODES[@]}"; do
          if [[ "$ps" == "text" ]]; then
            # text mode에서 exemplar_selection은 무의미
            tag="${ps}_${gm}_ss${ss}_st${st}_${wm}"
            CONFIGS+=("${tag}|${gm}|${ss}|${st}|${ps}|all|${wm}")
          else
            for es in "${EXEMPLAR_SELECTIONS[@]}"; do
              tag="${ps}_${es}_${gm}_ss${ss}_st${st}_${wm}"
              CONFIGS+=("${tag}|${gm}|${ss}|${st}|${ps}|${es}|${wm}")
            done
          fi
        done
      done
...
TOTAL=${#CONFIGS[@]}
echo "========================================="
echo " v19 Grid Search: $TOTAL configs"
echo " GPUs: ${GPU_IDS[*]}"
echo "========================================="
...
run_one() {
  local gpu_id=$1
  local config_str=$2
  IFS='|' read -r tag gm ss st ps es wm <<< "$config_str"
  ...
  CUDA_VISIBLE_DEVICES=$gpu_id $PYTHON generate_v19.py \
    --prompts "$PROMPTS" \
    --outdir "$out_dir" \
    --guide_mode "$gm" \
    --safety_scale "$ss" \
...
echo ""
echo "========================================="
echo " v19 Grid Search COMPLETE: $TOTAL configs"
echo "========================================="
