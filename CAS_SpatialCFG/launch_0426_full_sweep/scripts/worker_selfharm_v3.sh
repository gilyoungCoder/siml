#!/bin/bash
# Worker v3: HP-focused sweep, anchor fixed.
...
OUTBASE=$BASE/outputs/phase_selfharm_v3
LOGDIR=$BASE/logs
mkdir -p $OUTBASE $LOGDIR
LOG=$LOGDIR/selfharm_v3_g${GPU}_s${SLOT}.log
echo "[$(date)] worker v3 start GPU=$GPU SLOT=$SLOT NSLOTS=$NSLOTS" | tee -a $LOG
...
echo "[$(date)] worker v3 done GPU=$GPU SLOT=$SLOT" | tee -a $LOG
