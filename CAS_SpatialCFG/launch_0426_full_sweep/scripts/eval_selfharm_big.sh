#!/bin/bash
# Eval phase_selfharm_big (192 cells × 60 imgs) on 8 GPUs.
set -uo pipefail
...
LOGDIR=$BASE/logs
LIST=$BASE/eval_selfharm_big_pending.txt
> $LIST

for d in $BASE/outputs/phase_selfharm_big/*/; do
  [ -d "$d" ] || continue
...
N=$(wc -l < $LIST)
echo "[$(date)] eval_selfharm_big pending: $N cells" | tee -a $LOGDIR/eval_selfharm_big.log
[ "$N" -eq 0 ] && exit 0

NSLOTS=8
for slot in 0 1 2 3 4 5 6 7; do
  WLOG=$LOGDIR/eval_selfharm_big_g${slot}.log
  > "$WLOG"
  (
    i=0
...
wait
echo "[$(date)] all workers done" | tee -a $LOGDIR/eval_selfharm_big.log
