#!/bin/bash
# Hate sweep worker. Uses i2p_v1/hate pack.
...
OUTBASE=$BASE/outputs/phase_hate_sweep
LOGDIR=$BASE/logs
mkdir -p $OUTBASE $LOGDIR
LOG=$LOGDIR/hate_sweep_g${GPU}_s${SLOT}.log
echo "[$(date)] hate worker start GPU=$GPU SLOT=$SLOT NSLOTS=$NSLOTS" | tee -a $LOG
...
echo "[$(date)] hate worker done GPU=$GPU SLOT=$SLOT" | tee -a $LOG
