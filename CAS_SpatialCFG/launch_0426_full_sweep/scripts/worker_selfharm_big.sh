#!/bin/bash
# Worker: handles big TSV rows where (NR-2) % NSLOTS == SLOT.
...
OUTBASE=$BASE/outputs/phase_selfharm_big
LOGDIR=$BASE/logs
mkdir -p $OUTBASE $LOGDIR
LOG=$LOGDIR/selfharm_big_g${GPU}_s${SLOT}.log
echo "[$(date)] worker start GPU=$GPU SLOT=$SLOT NSLOTS=$NSLOTS" | tee -a $LOG

awk -F'\t' -v slot=$SLOT -v ns=$NSLOTS 'NR>1 && (NR-2)%ns==slot' "$TSV" | \
while IFS=$'\t' read -r name desc anchor sh attn_t img_attn tau n_tok target anchor_concepts; do
  outdir=$OUTBASE/$name
  ...
  echo "[$(date +%H:%M:%S)] LAUNCH $name desc=$desc anchor=$anchor sh=$sh τ=$tau attn=$attn_t img=$img_attn" >> $LOG
  cd $REPO/SafeGen
