#!/usr/bin/env python3
"""Plot nested image-count saturation: SR + Full% + NR% per concept × K."""
import re, csv
...
SRC  = BASE / "outputs/phase_img_sat_nested"
...
csv_path = OUT / "img_sat_nested_table.csv"
...
fig.suptitle("Image-count saturation (nested subsample, image-only probe, hybrid mode, n_tok=4)\n"
             "Dashed line at K=4 = paper default. Same 16-img source pool, K=N takes first N.", fontsize=13, y=0.998)
fig.tight_layout()
for ext in ("png","pdf"):
    p = OUT / f"img_sat_nested.{ext}"
