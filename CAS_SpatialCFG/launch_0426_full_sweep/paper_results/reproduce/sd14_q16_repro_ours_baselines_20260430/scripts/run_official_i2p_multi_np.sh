...
LOCKDIR="$ROOT/.locks/i2p_multi_${METHOD}_${MULTI}_${CONCEPT}.lock"
mkdir -p "$ROOT/.locks"
if ! mkdir "$LOCKDIR" 2>/dev/null; then echo "[SKIP_LOCKED] $METHOD $MULTI $CONCEPT lock=$LOCKDIR"; exit 0; fi
...
case "$MULTI" in
  2c) CONCEPTS="sexual violence" ;;
  3c) CONCEPTS="sexual violence self-harm" ;;
  7c) CONCEPTS="sexual violence self-harm shocking illegal_activity harassment hate" ;;
  *) echo bad multi; exit 2;;
esac
contains=0; for x in $CONCEPTS; do [ "$x" = "$CONCEPT" ] && contains=1; done
if [ "$contains" != 1 ]; then echo "[SKIP not in multi] $METHOD $MULTI $CONCEPT"; exit 0; fi
neg_for(){
 case "$1" in
...
case "$METHOD" in
 safedenoiser_multi)
  OUT=$ROOT/outputs/safedenoiser_multi_${MULTI}/i2p_q16/${CONCEPT}
  OREPO=$ROOT/code/official_repos/Safe_Denoiser
  SCRIPT=run_copro_i2p_concept_np.py
  TASK=$ROOT/configs/multi_official/safedenoiser_i2p_multi_${MULTI}.yaml
  ERASE=safree_neg_prompt_rep_threshold_time;;
 sgf_multi)
  OUT=$ROOT/outputs/sgf_multi_${MULTI}/i2p_q16/${CONCEPT}
  OREPO=$ROOT/code/official_repos/SGF/nudity_sdv1
  SCRIPT=generate_unsafe_sgf_i2p_concept_np.py
  TASK=$ROOT/configs/multi_official/sgf_i2p_multi_${MULTI}.yaml
...
if [ "$COUNT" -lt "$EXPECTED" ]; then
 ...
 cd "$OREPO"
 echo "RUN_MULTI method=$METHOD multi=$MULTI concept=$CONCEPT eval=$EVAL_CONCEPT gpu=$GPU out=$OUT"
