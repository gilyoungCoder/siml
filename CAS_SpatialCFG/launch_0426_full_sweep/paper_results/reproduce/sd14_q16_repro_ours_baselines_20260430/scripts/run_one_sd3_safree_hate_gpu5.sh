...
OUT=$ROOT/outputs/crossbackbone_0501/sd3/safree/i2p_q16/hate
PROMPT=$ROOT/prompts/i2p_q16_csv/hate_q16_top60.csv
mkdir -p "$OUT"
rm -rf "$OUT"; mkdir -p "$OUT"
CUDA_VISIBLE_DEVICES=5 PYTHONNOUSERSITE=1 "$PY" "$WRAP" "$GEN" \
  --method safree --prompts "$PROMPT" --outdir "$OUT" \
