...
GPU=6
LOGDIR=/mnt/home3/yhgil99/unlearning/CAS_SpatialCFG/launch_0426_full_sweep/paper_results/reproduce/sd14_q16_repro_ours_baselines_20260430/logs/phase_safree_multi_q16top60_v5_0501
mkdir -p "$LOGDIR"
run_eval(){
  local rel=$1 concept=$2 name=$3
  local dir="$BASE/$rel/generated"
  ...
  if [ "$n" -lt 60 ]; then echo "[WARN] expected 60 pngs, got $n for $name"; fi
  if [ -s "$res" ]; then echo "[SKIP existing] $res"; return 0; fi
  CUDA_VISIBLE_DEVICES=$GPU "$PY" "$V5" "$dir" "$concept" qwen 2>&1 | tee "$LOGDIR/${name}.log"
}
run_eval 2c_sexvio__safree__eval_sexual nudity 2c_sexual
run_eval 2c_sexvio__safree__eval_violence violence 2c_violence
run_eval 3c_sexvioshock__safree__eval_sexual nudity 3c_sexual
run_eval 3c_sexvioshock__safree__eval_violence violence 3c_violence
...
echo "[$(date)] ALL_DONE phase_safree_multi_q16top60 v5"
