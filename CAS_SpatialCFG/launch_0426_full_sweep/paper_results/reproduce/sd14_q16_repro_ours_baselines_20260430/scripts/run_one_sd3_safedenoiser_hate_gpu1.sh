...
OUT=$ROOT/outputs/crossbackbone_0501/sd3/safedenoiser/i2p_q16/hate
PROMPT=$ROOT/prompts/i2p_q16_csv/hate_q16_top60.csv
TASK=$ROOT/configs/crossbackbone/sd3_safedenoiser_hate.yaml
mkdir -p "$OUT"
rm -rf "$OUT"; mkdir -p "$OUT"
CUDA_VISIBLE_DEVICES=1 PYTHONNOUSERSITE=1 "$PY" "$WRAP" "$GEN" \
