...
def load_prompts(txt_path, nsamples=4):
    with open(txt_path) as f: prompts=[l.strip() for l in f if l.strip()]
...
def clip_score(img_dir, prompts, device='cuda', bs=32):
    ...
    with torch.no_grad():
        for i in range(0,len(files),bs):
            ...
            imf=image_features(model, ip['pixel_values']); txf=text_features(model, tp['input_ids'], tp['attention_mask'])
            imf=F.normalize(imf, dim=-1); txf=F.normalize(txf, dim=-1)
            sim=(imf*txf).sum(dim=-1)*100.0
            total += sim.sum().item(); count += sim.numel()
            if (i//bs)%5==0: print(f'  CLIP batch {i//bs+1}/{(len(files)+bs-1)//bs}', flush=True)
    return total/count
def fid(a,b,device='cuda'):
    return calculate_fid_given_paths([a,b], batch_size=64, device=device, dims=2048, num_workers=4)
if __name__=='__main__':
    base,meth,prompts_txt=sys.argv[1:4]; device='cuda' if torch.cuda.is_available() else 'cpu'
    prompts=load_prompts(prompts_txt,4); nb=len(imgs(base)); nm=len(imgs(meth))
    print(f'Baseline: {nb} | Method: {nm} | Prompts: {len(prompts)}')
    cb=clip_score(base,prompts[:nb],device); cm=clip_score(meth,prompts[:nm],device); fv=fid(base,meth,device)
    out=Path(meth)/'results_fid_clip_fixed.txt'
    out.write_text(f'Baseline dir: {base}\nMethod dir:   {meth}\nN_baseline: {nb}, N_method: {nm}\n\nCLIP Score (Baseline): {cb:.4f}\nCLIP Score (Method):   {cm:.4f}\nCLIP delta: {cm-cb:+.4f}\n\nFID (baseline vs method): {fv:.2f}\n')
    print(out.read_text())
