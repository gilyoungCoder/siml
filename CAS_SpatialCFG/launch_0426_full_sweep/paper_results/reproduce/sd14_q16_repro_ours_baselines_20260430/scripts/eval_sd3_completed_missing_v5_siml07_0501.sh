...
wait_free(){
  ...
  while true; do
    m=$(mem_used "$g")
    if [ "${m:-999999}" -lt 3000 ]; then return 0; fi
    echo "$(date +%F_%T) gpu=$g busy mem=$m"
    sleep 60
  done
}
worker(){
  ...
  while IFS="$(printf '\t')" read -r method sub name concept dir; do
    ...
    if [ -s "$res" ] || [ -s "$cat" ]; then echo "SKIP exists $method $sub $name" >> "$log"; continue; fi
    wait_free "$gpu" >> "$log" 2>&1
    echo "RUN $method $sub $name concept=$concept dir=$dir $(date)" >> "$log"
    CUDA_VISIBLE_DEVICES="$gpu" "$PY" "$VLM" "$dir" "$concept" qwen >> "$log" 2>&1
    echo "DONE rc=$? $method $sub $name $(date)" >> "$log"
    sleep 5
  done < "$JOB"
  echo "COMPLETE idx=$idx gpu=$gpu $(date)" >> "$log"
}
for i in "${!GPUS[@]}"; do
  worker "$i" "${GPUS[$i]}" &
  echo $! > "$ROOT/pids/eval_sd3_completed_missing_v5_gpu${GPUS[$i]}.pid"
done
wait
