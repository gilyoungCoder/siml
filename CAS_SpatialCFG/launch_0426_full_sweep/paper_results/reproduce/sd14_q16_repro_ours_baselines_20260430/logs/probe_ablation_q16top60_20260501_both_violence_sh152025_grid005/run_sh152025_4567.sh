...
AD=probe_ablation_q16top60_20260501_both_violence_sh152025_grid005
...
SUMMARY=$ROOT/summaries/probe_ablation_violence_both_sh152025_grid005_20260501.md
mkdir -p "$CFGDIR" "$OUTROOT" "$LOGDIR" "$ROOT/summaries"
echo "# Violence both/hybrid grid: sh in {15,20,25}; 0.05-step thresholds" > "$SUMMARY"
echo "GPU policy: siml-05 GPUs 4/5/6/7 only; GPU1 excluded." >> "$SUMMARY"
echo >> "$SUMMARY"
    ...
    cat > "$cfg" <<JSON
{
  ...
  "save_maps": false,
  "note": "sh15/20/25-only, 0.05-grid, violence both/hybrid"
}
JSON
    echo "[$(date)] START $name gpu=$gpu range=$start:$end sh=$sh tau=$tau txt=$txt img=$img" | tee -a "$LOGDIR/worker_sh152025_4567.log"
  ...
  echo "[$(date)] GEN_DONE $name n=$n" | tee -a "$LOGDIR/worker_sh152025_4567.log"
  CUDA_VISIBLE_DEVICES=4 "$PYVLM" "$V5" "$out" violence qwen > "$LOGDIR/${name}_eval.log" 2>&1
  cat "$out/results_qwen3_vl_violence_v5.txt" | tee -a "$LOGDIR/worker_sh152025_4567.log"
...
# Natural priority: sh20 and sh25 first, then sh15 for lower-collapse control.
run_candidate sh20_tau04_txt025_img010 20 0.40 0.25 0.10
run_candidate sh20_tau04_txt020_img010 20 0.40 0.20 0.10
run_candidate sh20_tau035_txt030_img010 20 0.35 0.30 0.10
run_candidate sh20_tau045_txt030_img010 20 0.45 0.30 0.10
run_candidate sh20_tau04_txt030_img015 20 0.40 0.30 0.15
run_candidate sh20_tau04_txt025_img015 20 0.40 0.25 0.15
...
run_candidate sh25_tau04_txt015_img010 25 0.40 0.15 0.10
...
run_candidate sh15_tau04_txt030_img010 15 0.40 0.30 0.10
run_candidate sh15_tau04_txt025_img010 15 0.40 0.25 0.10
run_candidate sh15_tau04_txt020_img010 15 0.40 0.20 0.10
run_candidate sh15_tau035_txt030_img010 15 0.35 0.30 0.10
run_candidate sh15_tau045_txt030_img010 15 0.45 0.30 0.10
python3 - "$SUMMARY" <<'PY' | tee -a "$LOGDIR/worker_sh152025_4567.log"
import sys,pathlib
