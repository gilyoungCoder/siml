...
run_one(){
  gpu=$1; start=$2; end=$3; cfg="$LOGDIR/${NAME}_shard_${start}_${end}.json"
  echo "[$(date)] START shard gpu=$gpu range=$start:$end" | tee -a "$LOGDIR/worker_siml05_4567_${NAME}.log"
  CUDA_VISIBLE_DEVICES=$gpu REPRO_ROOT=$CAND OUT_ROOT=$ROOT PY_SAFGEN=$PYGEN     "$PYGEN" "$CAND/scripts/run_from_config.py" --gpu "$gpu" --config "$cfg"     > "$LOGDIR/${NAME}_siml05_gpu${gpu}_${start}_${end}_gen.log" 2>&1 &
}
run_one 4 0 15
run_one 5 15 30
run_one 6 30 45
