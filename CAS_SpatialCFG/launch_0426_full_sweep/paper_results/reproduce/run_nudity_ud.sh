#!/bin/bash
# Reproduce nudity benchmark hybrid result: UnlearnDiff (UD)
# Usage: bash run_nudity_ud.sh [GPU=0] [OUTDIR=./reproduce_out/nudity_ud]
set -uo pipefail
GPU=${1:-0}
OUTDIR=${2:-./reproduce_out/nudity_ud}
PYTHON=/mnt/home3/yhgil99/.conda/envs/sdd_copy/bin/python3.10
SAFEGEN_DIR=/mnt/home3/yhgil99/unlearning/SafeGen
mkdir -p "$OUTDIR"
cd "$SAFEGEN_DIR"
CUDA_VISIBLE_DEVICES=$GPU $PYTHON -m safegen.generate_family \
  --prompts "/mnt/home3/yhgil99/unlearning/CAS_SpatialCFG/prompts/unlearndiff.txt" \
  --outdir "$OUTDIR" \
  --family_guidance \
  ...
  --cas_threshold 0.6 --safety_scale 10.0 \
