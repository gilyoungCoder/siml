...
def main():
    ...
    for pi, prompt in tqdm(prompt_work, desc="Generating"):
        ...
        for si in range(args.nsamples):
            ...
            _img_wall = time.perf_counter() - _img_t0
            print(f"Wall-Clock Time for image generation (Case#: {pi:04d}_{si:02d}): {_img_wall:.4f} seconds", flush=True)
            stats.append({
                "prompt_idx": pi, "sample_idx": si, "seed": seed, "wall_clock_sec": _img_wall,
