...
# -*- coding: utf-8 -*-
"""
Exemplar Subspace Projection v8: Training-Free Safe Generation
...
Architecture:
  ...
  HOW (Exemplar Projection): project online d_target onto exemplar concept subspace
      - exemplar_proj:       eps_safe = eps_cfg - s * M * proj(d_target, d_exemplar)
      - exemplar_proj_dual:  eps_safe = eps_cfg - t_s * M * proj(d_target, d_ex_tgt) + a_s * M * d_ex_anchor
      - Also supports all v4 guide_modes as fallback

Key idea:
  d_target (online, image-specific) captures "what the UNet thinks nudity looks like for THIS image"
  d_exemplar (offline) captures "what nudity looks like on average across diverse exemplars"
  proj(d_target, d_exemplar) = the component of d_target that aligns with the exemplar concept
  By removing ONLY this projected component, we:
    1. Remove nudity-related noise while preserving everything else
    2. Benefit from exemplar's concept knowledge (what IS nudity)
    3. Stay image-specific (d_target comes from current z_t)

UNet calls: 3 per guided step (null+prompt, target) — same as v7, 1 less than v4
"""
...
# =============================================================================
# Global CAS: Concept Alignment Score (WHEN) — same as v4
# =============================================================================
class GlobalCAS:
...
# =============================================================================
# Guidance Application (HOW) — v8 exemplar projection modes + v4 fallbacks
# =============================================================================
def apply_guidance_v8(
    ...
    **kwargs,
    ...
    """
    v8 guidance with exemplar subspace projection.

    New modes:
        exemplar_proj: Project d_target onto exemplar direction, remove only aligned component.
            d_concept = proj(d_target, d_exemplar_target)
            eps_safe = eps_cfg - s * M * d_concept

        exemplar_proj_dual: Projection removal + exemplar anchor addition.
            d_concept = proj(d_target, d_ex_target)
            eps_safe = eps_cfg - t_s * M * d_concept + a_s * M * d_ex_anchor

    Fallback modes (same as v4): sld, hybrid, projection, hybrid_proj
    """
    mask = soft_mask.to(eps_cfg.dtype)

    if guide_mode == "exemplar_proj":
        # Project online d_target onto exemplar concept direction
        d_target = (eps_target - eps_null).float()
        d_ex = d_exemplar_target.float()

        # Per-pixel projection: proj(d_target, d_exemplar)
        # dot product along channel dim (dim=1), keep spatial dims
        dot = (d_target * d_ex).sum(dim=1, keepdim=True)
        norm_sq = (d_ex * d_ex).sum(dim=1, keepdim=True).clamp(min=1e-8)
        d_concept = (dot / norm_sq) * d_ex  # component of d_target along d_exemplar

        eps_final = eps_cfg - safety_scale * mask * d_concept.to(eps_cfg.dtype)

    elif guide_mode == "exemplar_proj_dual":
        # Projection removal + exemplar anchor
        d_target = (eps_target - eps_null).float()
        d_ex_tgt = d_exemplar_target.float()
        d_ex_anc = d_exemplar_anchor.float()

        # Project d_target onto exemplar target direction
        dot = (d_target * d_ex_tgt).sum(dim=1, keepdim=True)
        norm_sq = (d_ex_tgt * d_ex_tgt).sum(dim=1, keepdim=True).clamp(min=1e-8)
        d_concept = (dot / norm_sq) * d_ex_tgt

        t_scale = kwargs.get("target_scale", safety_scale)
        a_scale = kwargs.get("anchor_scale", safety_scale)

        eps_final = eps_cfg \
                    - t_scale * mask * d_concept.to(eps_cfg.dtype) \
                    + a_scale * mask * d_ex_anc.to(eps_cfg.dtype)

    elif guide_mode == "exemplar_proj_prompt":
        # Project d_PROMPT (not d_target) onto exemplar concept direction
        # Then reconstruct safe CFG without nudity component
        d_prompt = (eps_prompt - eps_null).float()
        d_ex = d_exemplar_target.float()

        dot = (d_prompt * d_ex).sum(dim=1, keepdim=True)
        norm_sq = (d_ex * d_ex).sum(dim=1, keepdim=True).clamp(min=1e-8)
        proj_component = (dot / norm_sq) * d_ex

        p_scale = kwargs.get("proj_scale", 1.0)
        d_safe = d_prompt - p_scale * proj_component
        eps_safe_cfg = eps_null + cfg_scale * d_safe.to(eps_null.dtype)
        ...
        # Use exemplar anchor if available, otherwise fall back to online
        t_scale = kwargs.get("target_scale", safety_scale)
        a_scale = kwargs.get("anchor_scale", safety_scale)
        d_anchor = d_exemplar_anchor if d_exemplar_anchor is not None else torch.zeros_like(eps_null)
        eps_final = eps_cfg \
                    - t_scale * mask * (eps_target - eps_null) \
                    + a_scale * mask * d_anchor.to(eps_cfg.dtype)
...
def parse_args():
    p = ArgumentParser(description="Exemplar Subspace Projection v8")
    ...
    # Guidance (How) — v8 modes
    p.add_argument("--guide_mode", type=str, default="exemplar_proj",
                    choices=["exemplar_proj", "exemplar_proj_dual", "exemplar_proj_prompt",
                             "sld", "hybrid", "projection"],
                    help="Guidance mode")
    ...
    p.add_argument("--concept_dir_path", type=str, required=True,
                    help="Path to concept_directions.pt from prepare_concept_subspace.py")
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Exemplar Subspace Projection v8")
    print(f"{'='*70}")
    ...
    # ---- Load exemplar concept directions ----
    ...
    target_dirs = concept_data['target_directions']   # {t_int: [4,64,64]}
    anchor_dirs = concept_data['anchor_directions']   # {t_int: [4,64,64]}
    ...
    for prompt_idx, prompt in tqdm(prompts_with_idx, desc="Generating"):
        ...
        for sample_idx in range(args.nsamples):
            ...
            for step_idx, t in enumerate(scheduler.timesteps):
                ...
                if should_guide:
                    # Load exemplar directions for this timestep
                    ...
                    # Apply guidance (HOW) — v8 exemplar projection
                    eps_final = apply_guidance_v8(
                        eps_cfg=eps_cfg,
                        eps_null=eps_null,
    ...
    summary = {
        "method": "Exemplar Subspace Projection v8",
        "args": vars(args),
        "overall": {
            "total_images": n,
            "triggered": n_trig,
