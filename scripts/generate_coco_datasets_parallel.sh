#!/bin/bash
# ============================================================================
# Generate images for COCO 10k dataset - PARALLEL execution
# Each method runs on a different GPU
#
# Usage: bash generate_coco_datasets_parallel.sh [GPU_START]
# Example: bash generate_coco_datasets_parallel.sh 0
...
BASE_SCRIPT="${SCRIPT_DIR}/generate_coco_datasets.sh"
...
echo "=============================================="
echo "Parallel COCO 10k Dataset Generation"
echo "=============================================="
echo "Dataset: COCO 10k (nudity settings)"
echo ""
echo "GPU Assignment:"
...
LOG_DIR="/mnt/home/yhgil99/unlearning/logs/coco_$(date +%Y%m%d_%H%M%S)"
mkdir -p "${LOG_DIR}"
...
nohup bash "${BASE_SCRIPT}" ${GPU_BASELINE} baseline > "${LOG_DIR}/baseline.log" 2>&1 &
PID_BASELINE=$!
echo "[GPU ${GPU_BASELINE}] Baseline started (PID: ${PID_BASELINE})"

nohup bash "${BASE_SCRIPT}" ${GPU_SAFREE} safree > "${LOG_DIR}/safree.log" 2>&1 &
PID_SAFREE=$!
echo "[GPU ${GPU_SAFREE}] SAFREE started (PID: ${PID_SAFREE})"

nohup bash "${BASE_SCRIPT}" ${GPU_OURS} ours > "${LOG_DIR}/ours.log" 2>&1 &
PID_OURS=$!
echo "[GPU ${GPU_OURS}] Ours 4class started (PID: ${PID_OURS})"

nohup bash "${BASE_SCRIPT}" ${GPU_SAFREE_OURS} safree_ours > "${LOG_DIR}/safree_ours.log" 2>&1 &
PID_SAFREE_OURS=$!
