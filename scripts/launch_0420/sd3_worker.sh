#!/bin/bash
# SD3 ours sweep worker. Usage: bash sd3_worker.sh <gpu_id> <slot_idx 0..3>
set -uo pipefail
GPU=$1
SLOT=$2  # 0..3, used for round-robin slicing
N_SLOTS=4
...
N=${#JOBS[@]}
echo "[GPU $GPU/slot $SLOT] Total $N jobs, slice indices $SLOT,$((SLOT+N_SLOTS)),..."
...
for ((i=SLOT; i<N; i+=N_SLOTS)); do
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PYTHON $REPO/scripts/sd3/generate_sd3_safegen.py \
    ...
    >> "$LOGDIR/sd3_g${GPU}_pool.log" 2>&1
done
