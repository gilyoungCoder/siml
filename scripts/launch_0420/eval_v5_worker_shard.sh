#!/bin/bash
# v5-only sharded Qwen3-VL eval worker.
...
# v5 nudity rubric is unchanged from v3.
# Usage: bash eval_v5_worker_shard.sh <gpu_id> <shard_idx> <num_shards>
set -uo pipefail
GPU=$1
SHARD=$2
NSHARDS=$3
...
mkdir -p $LOGDIR
LOG="$LOGDIR/eval_v5_g${GPU}_s${SHARD}.log"
...
while true; do
  LOOP=$((LOOP+1))
  echo "[$(date)] [v5 shard $SHARD/$NSHARDS gpu $GPU] loop #$LOOP" | tee -a "$LOG"
  ...
  while IFS= read -r outdir; do
    ...
    cat_file_v5="$outdir/categories_qwen3_vl_${concept}_v5.json"
    if [ -f "$cat_file_v5" ] && [ -s "$cat_file_v5" ]; then
      v5_count=$(python3 -c "import json,sys; sys.stdout.write(str(len(json.load(open('$cat_file_v5')))))" 2>/dev/null || echo 0)
    ...
    cd $REPO/vlm
    echo "[$(date)] [GPU $GPU shard $SHARD] [v5] $rel concept=$concept n=$n_imgs" | tee -a "$LOG"
    if CUDA_VISIBLE_DEVICES=$GPU $PYTHON opensource_vlm_i2p_all_v5.py "$outdir" "$concept" qwen >> "$LOG" 2>&1; then
      PROCESSED=$((PROCESSED+1))
    else
      echo "  v5 FAILED $rel" | tee -a "$LOG"
      FAILED=$((FAILED+1))
    fi
  ...
  echo "[$(date)] [v5 shard $SHARD] loop #$LOOP done: processed=$PROCESSED skipped=$SKIPPED failed=$FAILED" | tee -a "$LOG"
  if [ "$PROCESSED" = "0" ] && [ "$FAILED" = "0" ] && [ "$LOOP" -gt 2 ]; then
    echo "[$(date)] [v5 shard $SHARD] no work, exiting" | tee -a "$LOG"
    break
  fi
  sleep 60
done
