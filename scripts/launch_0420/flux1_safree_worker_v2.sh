#!/bin/bash
# FLUX1 SAFREE v2 worker — paper-aligned: --safree_token_filter --safree_re_attention --svf
# v2 vs v1 fix: adds missing --svf flag (Sec 3.3 self-validation filter).
set -uo pipefail
GPU=$1
...
for job in "${JOBS[@]}"; do
  ...
  OUTDIR="$REPO/CAS_SpatialCFG/outputs/launch_0420/safree_flux1_v2/$DSET"
  TGT_KW=${TGT[$CONCEPT]}
  ...
  mkdir -p "$OUTDIR"
  echo "[run] FLUX1 SAFREE v2 $DSET concept=$CONCEPT"
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PYTHON $REPO/CAS_SpatialCFG/generate_flux1_safree.py \
    ...
    --svf \
    --device cuda:0 \
    >> "$LOGDIR/flux1_safree_v2_g${GPU}.log" 2>&1
done
echo "[GPU $GPU] FLUX1 SAFREE v2 worker done at $(date)"
