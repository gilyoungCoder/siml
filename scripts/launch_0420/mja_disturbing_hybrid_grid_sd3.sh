#!/bin/bash
# MJA-disturbing SD3 hybrid ours grid: ss∈{10,15,20} × imgthr∈{0.3,0.4,0.5} = 9 configs.
# Usage: bash mja_disturbing_hybrid_grid_sd3.sh <gpu_id> <slot_idx> <n_slots>
set -uo pipefail
GPU=$1
SLOT=$2
NSLOTS=$3
...
OUT_BASE="$REPO/CAS_SpatialCFG/outputs/launch_0420/ours_sd3/mja_disturbing"
...
for ((i=SLOT; i<N; i+=NSLOTS)); do
  ...
  LOG="$LOGDIR/mja_disturbing_sd3_hybrid_g${GPU}_s${SLOT}.log"
