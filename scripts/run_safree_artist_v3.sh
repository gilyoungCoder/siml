...
echo "=== SAFREE Artist v3 START $(date) ==="

run_artist() {
    local gpu=$1 artist=$2 concept=$3

    for seed in 42 123 456; do
        local outdir="$SAVE/artist_${artist}/seed_${seed}"
        mkdir -p "$outdir"
        [ "$(find $outdir -name '*.png' 2>/dev/null | wc -l)" -gt 25 ] && echo "[SKIP] $artist seed=$seed" && continue
    ...
    local total=$(find "$SAVE/artist_${artist}" -name '*.png' 2>/dev/null | wc -l)
    echo "[DONE GPU$gpu] artist_$artist ($total imgs total)"
}
...
echo "=== SAFREE Artist v3 COMPLETE $(date) ==="
