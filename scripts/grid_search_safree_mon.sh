#!/bin/bash
# ============================================================================
# Grid Search: SAFREE + Monitoring + Spatial CG
# Best from ringabell: mon=0.3, gs=12.5, bs=2.0, sp=0.5-0.5, hs=1.0 (SR=89.9%)
...
if [ "$USE_NOHUP" = true ]; then
    ...
    LOG_FILE="${BASE_DIR}/scripts/logs/grid_safree_mon_${TIMESTAMP}.log"
...
GRADCAM_STATS="${BASE_DIR}/SoftDelete+CG/gradcam_stats/nudity_4class"
SD_MODEL="CompVis/stable-diffusion-v1-4"
SCRIPT="generate_safree_monitoring.py"
...
PROMPTS[p4dn]="${BASE_DIR}/SAFREE/datasets/p4dn_16_prompt.csv"
...
# === Grid params (around best: mon=0.3, gs=12.5, hs=1.0, bs=2.0, sp=0.5-0.5) ===
...
# Count
TOTAL_PER_DS=$((${#MONITORING_THRESHOLDS[@]} * ${#GUIDANCE_SCALES[@]} * ${#HARMFUL_SCALES[@]} * ${#BASE_GUIDANCE_SCALES[@]} * ${#SPATIAL_THR_STARTS[@]} * ${#SPATIAL_THR_ENDS[@]}))
TOTAL=$((TOTAL_PER_DS * ${#DATASETS[@]}))

echo "=============================================="
echo "GRID SEARCH: SAFREE + Monitoring"
echo "=============================================="
...
for DS in "${DATASETS[@]}"; do
    ...
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    OUTPUT_BASE="${BASE_DIR}/SoftDelete+CG/scg_outputs/grid_safree_mon_${DS}_${TIMESTAMP}"
                ...
                for BS in "${BASE_GUIDANCE_SCALES[@]}"; do
                    for SP_S in "${SPATIAL_THR_STARTS[@]}"; do
                        for SP_E in "${SPATIAL_THR_ENDS[@]}"; do
                            ...
                            EXP_NAME="mon${MON}_gs${GS}_hs${HS}_bs${BS}_sp${SP_S}-${SP_E}"
