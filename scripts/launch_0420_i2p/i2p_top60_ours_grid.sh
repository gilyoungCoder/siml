#!/bin/bash
# I2P top60 (sweep60) ours grid search around best hybrid both config.
# 10 configs × 6 I2P concepts = 60 jobs.
# Usage: bash i2p_top60_ours_grid.sh <gpu_id> <slot_idx> <n_slots>
set -uo pipefail
GPU=$1
SLOT=$2
NSLOTS=$3
...
OUT_BASE=$REPO/CAS_SpatialCFG/outputs/launch_0420_i2p/ours_sd14_grid_v1pack
LOGDIR=$REPO/logs/launch_0420_i2p
mkdir -p $LOGDIR $OUT_BASE

# Per-concept target/anchor keywords (aligned with concepts_v2 keywords used in past MJA)
...
# 10 configs around best (hybrid, both probe)
CONFIGS=(
  "hybrid|10|0.1|0.3"
  "hybrid|10|0.1|0.4"
  "hybrid|10|0.1|0.5"
...
# Build full job list: concept × config
JOBS=()
...
echo "[$(date)] [g$GPU s$SLOT/$NSLOTS] $N total jobs, my slice = $SLOT,$(($SLOT+$NSLOTS)),..."
...
for ((i=SLOT; i<N; i+=NSLOTS)); do
  ...
  LOG="$LOGDIR/ours_grid_g${GPU}_s${SLOT}.log"

  n_imgs=$(ls -1 "$outdir"/*.png 2>/dev/null | wc -l)
  if [ "$n_imgs" -ge 60 ]; then
    echo "[$(date)] [g$GPU] [skip] $CONCEPT $HOW ss=$SS imgthr=$IMG_THR ($n_imgs imgs)" | tee -a "$LOG"
    continue
  fi

  mkdir -p "$outdir"
  if [ ! -f "$pack" ]; then
    echo "[g$GPU] [missing pack] $pack — skip $CONCEPT" | tee -a "$LOG"
    continue
  fi
  echo "[$(date)] [g$GPU] [run] $CONCEPT $HOW ss=$SS txt=$TXT_THR img=$IMG_THR -> $outdir" | tee -a "$LOG"
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PYTHON -m safegen.generate_family \
    ...
    --target_concepts $tgt \
    --anchor_concepts $anc \
    >> "$LOG" 2>&1 || echo "[g$GPU] FAILED $CONCEPT $HOW ss=$SS" | tee -a "$LOG"
done
