...
# Usage: bash violence_fullhard_v1pack.sh <gpu> <slot> <n_slots>
set -uo pipefail
...
OUT_BASE=$REPO/CAS_SpatialCFG/outputs/launch_0420_i2p_fullhard/ours_sd14_v1pack/violence
...
for ((i=SLOT; i<N; i+=N_SLOTS)); do
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PY -m safegen.generate_family \
    ...
    >> $LOGDIR/violence_fullhard_v1_${CFG}_g${GPU}.log 2>&1
done
