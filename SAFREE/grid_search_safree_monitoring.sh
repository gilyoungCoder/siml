#!/bin/bash
# ============================================================================
# Grid Search: SAFREE + Sample-Level Monitoring + Spatial CG
#
# Monitoring: P(harm) = CDF((heatmap.mean() - mu) / sigma)
# If P(harm) > monitoring_threshold → apply guidance
#
# Usage:
#   ./grid_search_safree_monitoring.sh                  # Run all experiments
#   ./grid_search_safree_monitoring.sh --dry-run        # Print commands only
#   ./grid_search_safree_monitoring.sh --nohup          # Run in background
#   ./grid_search_safree_monitoring.sh --num-gpus 4     # Use 4 GPUs
# ============================================================================
...
if [ "$USE_NOHUP" = true ]; then
    ...
    LOG_DIR="./results/grid_search_safree_mon_ringabell_${TIMESTAMP}"
    mkdir -p "${LOG_DIR}"
...
# Classifier (4-class from SoftDelete+CG)
...
# Monitoring thresholds (P(harm) threshold)
MONITORING_THRESHOLDS=(0.3 0.4 0.5 0.6 0.7)
...
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_BASE="./results/grid_search_safree_mon_ringabell_${TIMESTAMP}"
mkdir -p "${OUTPUT_BASE}/logs"

echo "=============================================="
echo "SAFREE + MONITORING GRID SEARCH"
echo "=============================================="
...
echo "  - monitoring_threshold: ${#MONITORING_THRESHOLDS[@]} values"
...
cat > "${OUTPUT_BASE}/config.json" << EOF
{
    ...
    "parameters": {
        ...
        "spatial_thresholds": ["0.7 0.3", "0.5 0.3", "0.5 0.5"],
    ...
    while true; do
        for ((gpu=0; gpu<NUM_GPUS; gpu++)); do
            ...
            if [ -z "$JOB_PID" ] || ! kill -0 "$JOB_PID" 2>/dev/null; then
                run_experiment $gpu $MT $GS $BS $SP_START $SP_END "$EXP_NAME" &
                RUNNING_JOBS[$gpu]=$!
