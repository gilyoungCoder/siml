...
# -*- coding: utf-8 -*-
"""
SAFREE++ Violence 13-Class Spatial Classifier Guidance

Combines SAFREE text projection + Violence 13-class spatial guidance.

13-Class Structure:
  - Class 0: benign
  - Triplets (harm, safe, artifact):
    * fighting: (1, 2, 3)
    * weapon: (4, 5, 6)
    * blood: (7, 8, 9)
    * war: (10, 11, 12)
...
Harm-Safe Pairs:
  - harm_fighting(1) -> safe_fighting(2)
  - harm_weapon(4) -> safe_weapon(5)
  - harm_blood(7) -> safe_blood(8)
  - harm_war(10) -> safe_war(11)
"""
...
def f_beta(z: float, btype: str = "sigmoid", upperbound_timestep: int = 10, concept_type: str = "violence") -> int:
    t = 5.333
    k = 2.5
...
# Violence negative prompt space
VIOLENCE_NEGATIVE_PROMPTS = [
    ...
    "molotov cocktail", "arson", "war", "battle", "combat", "weapon"
]
...
# =========================
# Dynamic Harm Class Detector (13-class)
# =========================
class DynamicHarmClassDetector13:
    """Detects which harm class to guide from for violence 13-class."""
    ...
    def detect_harm(
    ...
    ) -> Tuple[bool, Optional[int], Optional[int], Dict]:
        """Detect which harm/artifact class is most active."""
        with torch.no_grad():
            ...
            if is_harmful:
                ...
                harm_class = max_class
                safe_class = self.artifact_to_safe[max_class]
            ...
            should_guide = is_harmful or is_artifact
        ...
        return should_guide, harm_class, safe_class, info


# =========================
# GradCAM Stats Loader (13-class)
# =========================
def load_gradcam_stats_map_13class(stats_dir: str) -> Dict[int, Dict[str, float]]:
    """Load per-class GradCAM statistics for 13-class."""
    stats_dir = Path(stats_dir)
    stats_map: Dict[int, Dict[str, float]] = {}

    # harm and artifact classes
    all_classes = [1, 3, 4, 6, 7, 9, 10, 12]
...
# =========================
# Selective Spatial Mask Generator (13-class)
# =========================
class SelectiveSpatialMaskGenerator13:
    def __init__(
        self,
        classifier_model,
        harm_detector: DynamicHarmClassDetector13,
    ...
    def generate_mask(
        ...
        should_guide, harm_class, safe_class, detection_info = self.harm_detector.detect_harm(
            latent=latent,
            timestep=timestep
        )
        ...
        if not should_guide or harm_class is None:
        ...
        if self.debug:
            print(f"  [Step {current_step}] {detection_info['max_class_name']} -> safe(class {safe_class}) - "
...
# =========================
# Selective Spatial Guidance (13-class)
# =========================
class SelectiveSpatialGuidance13:
    def __init__(
        self,
        classifier_model,
        device: str = "cuda",
...
# =========================
# SAFREE++ Violence 13-Class Pipeline
# =========================
class SAFREEViolence13ClassPipeline:
    def __init__(
        self,
        pipe: StableDiffusionPipeline,
        classifier_model,
        mask_generator: SelectiveSpatialMaskGenerator13,
        guidance_module: SelectiveSpatialGuidance13,
    ...
    def _encode_prompt(self, prompt, num_images_per_prompt, do_classifier_free_guidance, negative_prompt):
        ...
        text_embeddings = self.text_encoder(text_input_ids.to(self.device), attention_mask=None)[0]
        ...
        if do_classifier_free_guidance:
            ...
            uncond_embeddings = self.text_encoder(uncond_input.input_ids.to(self.device), attention_mask=None)[0]
    ...
    def __call__(
        ...
        negative_prompt_space = VIOLENCE_NEGATIVE_PROMPTS
            ...
            if svf_enabled:
                ...
                if proj_ort_e_act.numel() == 0 or text_e_act.numel() == 0:
                    ...
                    if math.isnan(beta):
                        ...
                        beta_adjusted = f_beta(beta, upperbound_timestep=svf_up_t, concept_type="violence")
        ...
        for i, t in enumerate(tqdm(timesteps, desc="SAFREE++ Violence 13-Class")):
...
# =========================
def parse_args():
    parser = ArgumentParser(description="SAFREE++ Violence 13-Class Spatial CG")

    parser.add_argument("--ckpt_path", type=str, default="CompVis/stable-diffusion-v1-4")
    parser.add_argument("--prompt_file", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="./results/safree_violence_13class")
...
def main():
    ...
    print("\n" + "="*80)
    print("SAFREE++ VIOLENCE 13-CLASS SPATIAL CG")
    print("="*80)
    ...
    if args.gradcam_stats_dir:
        gradcam_stats_map = load_gradcam_stats_map_13class(args.gradcam_stats_dir)
    ...
    pipeline = SAFREEViolence13ClassPipeline(
        pipe=pipe,
        classifier_model=classifier,
