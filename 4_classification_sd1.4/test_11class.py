...
def main(model=None):
    ...
    ########################
    if args.freedom:
        # Initialize the GuidanceModel with trained discriminator target class = 1
        ...
        def pre_freedom_iteration(diffusion_pipeline, step, timestep, callback_kwargs, freedom_model, freedom_scale, guide_start=0):
            ...
            if guide_start <= step:
                ...
                guidance_result = freedom_model.guidance(diffusion_pipeline, callback_kwargs, step, timestep, freedom_scale, target_class = 1 )
