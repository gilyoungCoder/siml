...
# (필요한 추가 import: IPAdapterMixin, FromSingleFileMixin, etc. 
#  원본 StableDiffusionXLPipeline 상속 시 이미 처리됩니다.)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        (
            ...
            lora_scale=None, clip_skip=self.clip_skip,
        )
        ...
        # 8) SDXL micro-conditioning embeddings 준비
        text_proj_dim = (
            pooled_prompt_embeds.shape[-1] if self.text_encoder_2 is None
        ...
        if negative_original_size is not None and negative_target_size is not None:
            neg_time_ids = self._get_add_time_ids(
                ...
                dtype=prompt_embeds.dtype, text_encoder_projection_dim=text_proj_dim
            )
        else:
        ...
        if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
            image_embeds = self.prepare_ip_adapter_image_embeds(
                ip_adapter_image, ip_adapter_image_embeds,
                device, batch_size, self.do_classifier_free_guidance
            )
        
        # 9) denoising loop — 여기서 prev_latents, noise_pred, latents 콜백 전달
        prev_latents = None
        with self.progress_bar(total=num_inference_steps) as progress:
            for i, t in enumerate(timesteps):
                ...
                # 9.1) classifier-free guidance 확장
                latent_input = (
                ...
                noise_pred = self.unet(
                    ...
                    timestep_cond=(
                        self.get_guidance_scale_embedding(
                            torch.tensor(self.guidance_scale - 1).repeat(batch_size),
                            embedding_dim=self.unet.config.time_cond_proj_dim
                        ).to(device, latents.dtype)
                        if self.unet.config.time_cond_proj_dim is not None else None
                    ),
                    cross_attention_kwargs=self.cross_attention_kwargs,
                    added_cond_kwargs=dict(
                        text_embeds=add_text_embeds,
                        time_ids=add_time_ids,
                        **({"image_embeds": image_embeds} if ip_adapter_image is not None or ip_adapter_image_embeds is not None else {})
                    ),
                    return_dict=False
                )[0]
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if self.guidance_rescale > 0:
                        noise_pred = self.rescale_noise_cfg(noise_pred, cond, self.guidance_rescale)
