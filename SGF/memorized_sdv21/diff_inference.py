...
def main(args):
    ...
    if args.modelstyle == "nolevel":
        ...
        '''
        m = len(objects)
        q, r = divmod(num_batches, m)    # 앞 r개는 q+1개, 나머지는 q개

        # 각 object의 반복 횟수 (편차 최대 1)
        counts = [q + (1 if i < r else 0) for i in range(m)]

        # 객체명을 블록으로 이어 붙임 (순서 유지, 셔플 X)
        temp = [obj for i, obj in enumerate(objects) for _ in range(counts[i])]

        # 프롬프트
        prompt_list = [f"An image of {x}" for x in temp]

        # 시드: 각 object 블록마다 0..(블록길이-1)로 순차 할당
        #  - 블록 길이가 q면 0..q-1
        #  - 블록 길이가 q+1이면 0..q  (즉, 0부터 num_batches/10까지)
    ...
    for i in range(num_batches):
        ...
        if args.modelpath is None:
            ...
            images = pipe(prompt=prompt, 
                          ...
                          num_images_per_prompt=args.im_batch).images
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nickname', type=str, default="", help="run nickname (빈 문자열이면 현재 시각으로 자동 설정)")
