...
class SynthDataset(Dataset):
    def __init__(self, main_dir, transform, capjson=None, imagenette_class=False):
        ...
        if not os.path.exists(f"{main_dir}/prompts.txt"): # TODO: will not work for imagenettemodel
            ...
            if 'laion' in main_dir or 'l100kaion' in main_dir:
                ...
                if imagenette_class is True:
                    capjson = glob.glob(os.path.join(p, "*class.json"))[0]
                else:
                    capjson = glob.glob(os.path.join(p, "*nolevel.json"))[0]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if "imagenette" in args.val_dir:
        if 'class' in args.query_dir:
            dataset_values = SynthDataset(args.val_dir, ret_transform, imagenette_class=True) # the train data
            dataset_simpl = SynthDataset(args.val_dir, 
                                         transforms.Compose([
                                            transforms.Resize(224),
                                            transforms.CenterCrop(224),
                                            transforms.ToTensor()]),
                                         imagenette_class=True
                                        ) 
    ...
    if args.rank == 0:  # only rank 0 will work from now on
        ...
        dp  = os.sep.join(os.path.normpath(args.query_dir).split(os.sep)[-4:])
         
        if not args.noeval: 
            import wandb
            wandb.init(project="imsimv2_retrieval",
                       name=f"{dp}_{args.similarity_metric}",
                       mode="offline",
                       dir="./wandb")
        ...
        print(simscore_results)

        # Save logs    
        clean_results = {k: float(v) for k, v in simscore_results.items()}
       ...
       ## simscores of repeated images
        ...
        log_path = os.path.join(
            f"ret_plots/{dp}/images/{args.pt_style}_{args.arch}_{args.similarity_metric}{args.stype}",
            "log.txt"
        )

        with open(log_path, "w") as f:
            f.write("Simscores @x% part done\n")
            ...
            # CLIP
            f.write(f"CLIP: {clipscore.item():.6f}\n")
            f.write(f"CLIP_BG: {clipscore_bg.item():.6f}\n")

            # wandb.log에 넣는 값들도 파일에 기록
            metrics = {
                'cc_ent': cc_ent,
                'pval_ent': pval_ent,
                'cc_comp': cc_comp,
