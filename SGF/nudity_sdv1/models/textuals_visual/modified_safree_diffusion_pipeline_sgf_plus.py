...
class ModifiedSafreeDiffusionPipeline_Rep_plus(StableDiffusionPipeline):
    def __init__(self,
        vae,
        text_encoder,
        tokenizer,
        ...
        super(ModifiedSafreeDiffusionPipeline_Rep_plus, self).__init__(
                vae,
                text_encoder,
                tokenizer,
                unet,
        ...
        self.scheduler_sgf = None
        
    ############
    # SGF PLUS #
    ############
    def _prepare_extra_step_kwargs_sgf(self, scheduler, generator, eta):
    ...
    def set_scheduler_sgf(self, scheduler_sgf):
        self.scheduler_sgf = scheduler_sgf
        self.scheduler_sgf_kwargs = self._prepare_extra_step_kwargs_sgf(self.scheduler_sgf, None, 0.0)
    ############
    ...
    def __call__(
        ...
        **kwargs
        ...
        """
        if kwargs.get("early_stop", False):
            negation_warmup_start = kwargs.get("negation_warmup_start", 1000)
            negation_warmup_end = kwargs.get("negation_warmup_end", 800)
        else:
            negation_warmup_start = kwargs.get("negation_warmup_start", 1000)
            negation_warmup_end = kwargs.get("negation_warmup_end", 55)
            ...
            for i, t in enumerate(timesteps):
                ...
                if (t <= negation_warmup_start) and (t >= negation_warmup_end): # Warmup
                    if repellency_processor is not None:
                        ...
                        # scheduler
                        if self.scheduler_sgf is not None:
                            x_0_hat = self.scheduler_sgf.step(noise_pred, t, latents, **self.scheduler_sgf_kwargs).pred_original_sample
                        else:
                            x_0_hat = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs).pred_original_sample
                        
                        repellency_dict = repellency_processor.conditioning(x_0_hat,
                                                                            beta_threshold=False)                        
                        x_0_hat_repellenced = repellency_dict['x_0_hat']
                    
                        noise = torch.randn(x_0_hat_repellenced.shape, 
                                            generator=generator, 
                                            device=device, dtype=torch.float32)
                        
                        # update latents with repellency && # scheduler
                        if self.scheduler_sgf is not None:
                            latents = self.scheduler_sgf.add_noise(x_0_hat_repellenced, noise, t)
                        else:
                            latents = self.scheduler.add_noise(x_0_hat_repellenced, noise, t)
