...
# -*- coding: utf-8 -*-
"""
Train an IMAGE-SPACE z0 9-class I2P classifier for SD1.4 guidance.

Pipeline:
  image -> VAE.encode() -> z0 -> noise -> zt
  -> UNet(zt, t, uncond) [frozen] -> noise_pred
  -> Tweedie -> z0_hat [detach]
  -> VAE.decode(z0_hat) -> x0_hat (3x512x512) [detach]
  -> ImageClassifier(x0_hat) -> logits -> CE loss
"""
...
class I2P9ClassDataset(Dataset):
    ...
    def __init__(self, class_dirs, transform=None, balance_classes=True, seed=42):
        ...
        class_paths = {}
        for cid, d in class_dirs.items():
            if os.path.exists(d):
                class_paths[cid] = sorted([
                    os.path.join(d, f) for f in os.listdir(d)
                    if os.path.splitext(f)[1].lower() in self.EXTENSIONS
                ])
            else:
                print(f"  Warning: not found class {cid} ({CLASS_NAMES.get(cid,'?')}): {d}")
                class_paths[cid] = []
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train image-space z0 9-class I2P classifier"
    )
    ...
    parser.add_argument("--wandb_project", type=str,
                        default="z0_img_i2p_9class_classifier")
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = f"./work_dirs/z0_img_{args.concept_name}_9class"
    if args.wandb_run_name is None:
        args.wandb_run_name = f"z0_img_{args.concept_name}_9class"

    os.makedirs(args.output_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"[Z0-IMG] Training 9-class classifier for: {args.concept_name.upper()}")
    print(f"{'='*60}\n")
    ...
    full_dataset = I2P9ClassDataset(
        class_dirs=class_dirs, transform=transform,
        ...
        for batch in train_loader:
            ...
            if global_step % args.save_ckpt_freq == 0:
                ...
                if args.use_wandb:
                    ...
                    for cid in range(NUM_CLASSES):
                        if class_total[cid] > 0:
                            log_dict[f"val_acc_{CLASS_NAMES[cid]}"] = class_correct[cid] / class_total[cid]
    ...
    print(f"\n[Z0-IMG] Complete: {args.concept_name.upper()} -> {args.output_dir}\n")
