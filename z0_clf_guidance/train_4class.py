...
# -*- coding: utf-8 -*-
"""
Train a z0-space 4-class classifier for SD1.4 latent classifier guidance.
...
Pipeline (each iteration):
  image -> VAE.encode() -> z0 (clean latent)
  -> add noise at random timestep t -> zt
  -> SD1.4 UNet(zt, t, uncond_emb) [frozen, no_grad] -> noise_pred
  -> z0_hat = Tweedie(zt, noise_pred, alpha_bar) [detach]
  -> Classifier(z0_hat) -> logits -> CE loss -> optimize classifier
"""
...
from utils.dataset import FourClassFolderDataset
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train z0-space 4-class classifier (benign/clothed/nude/color)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--harm_color_data_path", type=str, nargs="+", required=True,
                        help="Directory(ies) of color artifact (class 3) images")
    # Training
    parser.add_argument("--output_dir", type=str, default="work_dirs/z0_resnet18_4class")
    ...
    parser.add_argument("--wandb_project", type=str, default="z0_classifier")
    parser.add_argument("--wandb_run_name", type=str, default="z0_4class")
...
def main():
    ...
    # ================================================================
    # Frozen components: VAE, UNet, text encoder
    # ================================================================
    ...
    # Pre-compute unconditional text embedding (empty string)
    ...
    while global_step < max_steps:
        for batch in train_loader:
            ...
            # Step 1: VAE encode -> z0
            with torch.no_grad():
                z0 = vae.encode(imgs).latent_dist.sample() * 0.18215

            # Step 2: Random timestep & noise injection -> zt
            ...
            # Step 3: UNet one-step denoising (frozen)
            with torch.no_grad():
            ...
            # Step 4: Tweedie -> z0_hat (detached from UNet graph)
            z0_hat = predict_z0(zt, noise_pred, alpha_bar).detach()

            # Step 5: Classify z0_hat
            ...
            if args.use_wandb and global_step % 10 == 0:
                ...
                for ci in range(args.num_classes):
                    mask = (labels == ci)
                    if mask.sum() > 0:
                        log_dict[f"train_acc_{CLASS_NAMES[ci]}"] = (
            ...
            if global_step % args.save_ckpt_freq == 0:
                ...
                per_class_correct = [0] * args.num_classes
                per_class_total = [0] * args.num_classes
                ...
                with torch.no_grad():
                    for vb in val_loader:
                        ...
                        for ci in range(args.num_classes):
                            mask = (vlabels == ci)
                            per_class_correct[ci] += (vpreds[mask] == vlabels[mask]).sum().item()
                ...
                per_class_str = " | ".join(
                    f"{CLASS_NAMES[ci]}={per_class_correct[ci]/max(per_class_total[ci],1):.3f}"
                    for ci in range(args.num_classes)
                )
                logger.info(
                    f"[Val] step={global_step}, loss={val_loss:.4f}, acc={val_acc:.4f} | {per_class_str}"
                )
                ...
                if args.use_wandb:
                    ...
                    for ci in range(args.num_classes):
                        if per_class_total[ci] > 0:
                            log_dict[f"val_acc_{CLASS_NAMES[ci]}"] = (
