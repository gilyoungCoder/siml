#!/usr/bin/env bash
# ============================================================================
# [INFERENCE] Image-space SPATIAL PAIRED guidance — ALL 7 I2P concepts
...
for thresh in ${THRESHOLDS}; do
    ...
    echo "=============================================="
    echo "[img spatial paired] threshold=${thresh}  scale=${GUIDE_SCALE}  (tag=t${tag})"
    echo "=============================================="
    ...
    for concept in violence harassment hate illegal selfharm shocking sexual; do
        ...
        outdir="./output_img/i2p_${concept}_img_spatial_t${tag}"
        ...
        CUDA_VISIBLE_DEVICES=${gpu} nohup python generate.py "${CKPT_PATH}" \
          ...
          > "logs/infer_i2p_img_spatial_t${tag}_${concept}.log" 2>&1 &
...
echo "=============================================="
echo "All image-space spatial paired inference complete! (thresholds: ${THRESHOLDS})"
echo "=============================================="
