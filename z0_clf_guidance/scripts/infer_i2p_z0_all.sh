#!/usr/bin/env bash
# ============================================================================
# [INFERENCE] Latent z0-space classifier guidance — ALL 7 I2P concepts
...
echo "Launching all I2P latent z0-space inference jobs (high_tox, 50 prompts each)..."
echo ""
...
for concept in violence harassment hate illegal selfharm shocking sexual; do
    ...
    outdir="./output_img/i2p_${concept}_z0_safe_minus_harm"
    ...
    CUDA_VISIBLE_DEVICES=${gpu} nohup python generate.py "${CKPT_PATH}" \
      ...
      > "logs/infer_i2p_z0_${concept}.log" 2>&1 &

    echo "[GPU ${gpu}] ${concept} (z0, 9-class, 50 prompts) started. PID=$!"
done

echo ""
echo "=============================================="
echo "All 7 I2P latent z0-space inference jobs launched!"
echo "  tail -f logs/infer_i2p_z0_*.log"
echo "=============================================="
