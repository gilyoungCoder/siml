#!/usr/bin/env bash
# ============================================================================
# [INFERENCE] Image-space PAIRED classifier guidance — ALL 7 I2P concepts
# Prompts: *_high_tox.txt (50 each), 9-class ResNet18 on decoded x0_hat (3ch)
...
for scale in ${SCALES}; do
    ...
    echo "=============================================="
    echo "[img paired] guidance_scale=${scale}  (tag=s${tag})"
    echo "=============================================="
    ...
    for concept in violence harassment hate illegal selfharm shocking sexual; do
        ...
        outdir="./output_img/i2p_${concept}_img_paired_s${tag}"
        ...
        CUDA_VISIBLE_DEVICES=${gpu} nohup python generate.py "${CKPT_PATH}" \
          ...
          > "logs/infer_i2p_img_paired_s${tag}_${concept}.log" 2>&1 &
...
echo "=============================================="
echo "All image-space paired inference complete! (scales: ${SCALES})"
echo "=============================================="
