#!/bin/bash
# ============================================================================
# Grid Search V3: Z0 ASCG — Optimized monitoring (Step 13, mon=0.04)
#
# Fixed parameters (confirmed from analysis):
#   sticky_trigger = True
#   grad_clip_ratio = 0.0
#   monitoring_threshold = 0.04
...
#
# Server split (3 servers × 8 GPUs):
#   --server 0: Fast server  — 268 configs (indices 0-267)
#   --server 1: Slow server  — 134 configs (indices 268-401)
#   --server 2: Slow server  — 134 configs (indices 402-535)
#
# Usage:
#   bash scripts/run_grid_v3.sh --dataset ringabell --server 0              # dry run
#   bash scripts/run_grid_v3.sh --dataset ringabell --server 0 --run        # execute
#   bash scripts/run_grid_v3.sh --dataset ringabell --server 0 --run --nohup
# ============================================================================
...
if [ "$SERVER_ID" -lt 0 ] || [ "$SERVER_ID" -gt 2 ]; then
    echo "ERROR: --server is required (0=fast, 1=slow, 2=slow)"
    exit 1
fi
...
# Fixed params (confirmed from analysis)
NUM_STEPS=50
SEED=42
NSAMPLES=1
CFG_SCALE=7.5
...
OUTPUT_BASE="./grid_v3_output/${DATASET}"
...
# ============================================
case $SERVER_ID in
    0) COMBO_START=0;   COMBO_END=268;;
    1) COMBO_START=268; COMBO_END=402;;
    2) COMBO_START=402; COMBO_END=${TOTAL};;
esac

SERVER_COUNT=$((COMBO_END - COMBO_START))

echo "=============================================="
echo "Z0 ASCG GRID SEARCH V3 (optimized monitoring)"
echo "=============================================="
echo "Dataset: $DATASET"
echo "Server: $SERVER_ID (indices ${COMBO_START}-$((COMBO_END-1)))"
echo "Configs: $SERVER_COUNT / $TOTAL total"
echo "Fixed: mon=${MON_THR} start_step=${MON_START_STEP} sticky=T cl0=0 hr=${HARM_RATIO}"
