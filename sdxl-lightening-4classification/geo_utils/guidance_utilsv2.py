...
from geo_utils.gradient_model_utilsv2 import GradientModel
from geo_utils.gradient_model_utilsv2 import ClassifierGradientModel
from geo_utils.gradient_model_utilsv2 import YOLOGradientModel
from geo_utils.gradient_model_utilsv2 import DiscriminatorGradientModel,TimeDependentDiscriminatorGradientModel
from geo_utils.gradient_model_utilsv2 import VAALGradientModel, TimeDependentVAALGradientModel
from geo_utils.gradient_model_utilsv2 import AugmentedDiscriminatorGradientModel
from geo_utils.gradient_model_utilsv2 import ObjectDetectionDiscriminatorGradientModel
from geo_utils.gradient_model_utilsv2 import LatentCLIPGradientModel



class GuidanceModel:
    gradient_model: GradientModel
    def __init__(self, diffusion_pipeline, model_config_file, model_ckpt_paths, target_class, device="cpu"):
        ...
        # model_ckpt_paths: ["ckpt1.pth", "ckpt2.pth", "ckpt3.pth"]
        self.gradient_models = []
        for ckpt in model_ckpt_paths:
           gm = self.load_gradient_model(model_type, model_args)
           gm.load_model(ckpt)
           # all models on same device, FP32
           gm.model = gm.model.to(device).float()
           self.gradient_models.append(gm)
        self.gradientmodel = self.gradient_models[0]
    ...
    def load_gradient_model(self, model_type, model_config_obj):
        ...
        elif model_type == "latent_clip":
            # model_config_obj 은 LatentCLIPGradientModel.__init__ 에 넘길 dict
            return LatentCLIPGradientModel(model_config_obj, self.device)
    ...
    def guidance(self, diffusion_pipeline, callback_kwargs, step, timestep, scale, target_class = None):
        with torch.enable_grad():
            ...
            # 1) 각 모델별 grad, diff_value 수집
            grads = []
            diff_vals = []
            for gm in self.gradient_models:
                dv, g, _ = gm.get_grad(
                    scaled_latent, normalized_timestep, encoder_hidden_states, prev_latents, target_class=tc
                )
                grads.append(g)
                diff_vals.append(dv)

            # 2) 평균 그래디언트 및 평균 스칼라값 계산
            avg_grad = torch.stack(grads, dim=0).mean(dim=0)
            avg_diff = sum(diff_vals) / len(diff_vals)

            grad = avg_grad
            return_diff_value = avg_diff.detach()
        grad = grad.detach()
