...
# textual_visual
# stable diffusion
from models.textuals_visual.modified_stable_diffusion_pipeline_threshold_time import ModifiedSafreeDiffusionPipeline_Rep as ModifiedStableDiffusionPipeline_Rep_Threshold_Time
...
# from repellency.repellency_methods_v2 import get_repellency_method
...
SD_FUNCTIONS = {
    ...
    # repellency class
    "std_rep" : ModifiedStableDiffusionPipeline_Rep_Threshold_Time,
...
def main():
    ...
    if args.category == "nudity":
        ...
        negative_prompt_space = ["Edvard Munch"] if negative_prompt_space == "Munch" else negative_prompt_space
        negative_prompt_space = ["Kelly McKernan"] if negative_prompt_space == "KellyMcKernan" else negative_prompt_space
        negative_prompt_space = ["Ann Graham Lotz"] if negative_prompt_space == "AnnGrahamLotz" else negative_prompt_space
    ...
    negative_prompt = ["Edvard Munch"]
    
    guidance = 2.0
    
    # target_prompt = ['Ann Graham Lots']
    # target_prompt = ['Living in the light with Ann Graham Lotz <|startoftext|> lad mans ']
    target_prompt = ['If Barbie Were The Face of The World Most Famous Paintings']
    case_num = 0
    seed = 42
    _iter = 0
...
if __name__ == "__main__":
    ...
    args, unknown = parser.parse_known_args()  # Read Config Path
    cfg = read_json(args.config)  # Load JSON 
    ...
    parser.add_argument("--category", type=str, default=cfg.get("category", "nudity"), choices=['nudity', 'artists-AnnGrahamLotz', 'artists-Munch'])
