...
# ★ SOT(=BOS) 소프트 억제 제외 마스크 생성 (EOT는 건들지 않음)
@torch.no_grad()
...
def debug_print_prompt_cos(
    ...
    print(f"{'idx':>3} {'token':>18} {'is_used':>7} {'harm_cos':>9} {'allow_max':>10} {'best_allow':>16} {'suppress?':>10} {'soft_w':>8}")
    print("-" * 96)
    ...
    for i, use in enumerate(raw_mask.tolist()):
        ...
        suppress = (harm_cos >= tau) and (allow_max <= allow_tau)
        soft_w = max(0.0, harm_cos - allow_max) if suppress else 0.0  # ★ (harm - allow)

        is_used = bool(mask[i].item())
        print(f"{i:>3} {token:>18} {str(is_used):>7} {harm_cos:+.3f} {allow_max:+.3f} {best:>16} {str(suppress):>10} {soft_w:>8.3f}")
...
class HarmfulAttnProcessor(AttnProcessor2_0):
    ...
    - Soft delete: scores -= γ * max(0, harm_cos - allow_max) if (harm_cos >= τ and allow_max <= allow_τ)
    - Hard block:  scores := -∞ for tokens over τ_hard (if not allow) [기존 로직 유지]
    """
    def __init__(
        self,
        harm_vec: Optional[torch.Tensor],
        ...
        # ★ soft delete에서 제외할 토큰(열) 마스크(SOT 등)
    ...
    def __call__(
        ...
        if is_cross and self.cfg.enable and (harm_vec is not None):
            ...
            if (allow_mat is not None) and allow_mat.numel() > 0:
                ...
                allow_max = sims.amax(dim=-1)                          # (B, K)
                # 기존 safe(면제) 정의 유지: allow_max >= allow_tau
                safe_mask = (allow_max >= self.cfg.allow_tau)
            else:
                allow_max = torch.zeros_like(cos_harm)                 # allow가 없으면 0

            # ====== ★ Soft delete (변경) ======
            # 조건: harm>=τ AND allow<=allow_τ
            cond_soft = (cos_harm >= self.cfg.tau) & (allow_max <= self.cfg.allow_tau)  # (B,K)

            # 소프트 제외(SOT 등) 적용: 해당 열은 soft delete 미적용
            if self._soft_exempt_mask is not None:
                exc = self._soft_exempt_mask.to(cond_soft.device)  # (B,K) or (1,K)
                if exc.shape[0] == 1:
                    exc = exc.expand(B, -1)
                cond_soft = cond_soft & (~exc)

            # 가중치: γ * max(0, harm - allow)
            if cond_soft.any():
                # weight = (cos_harm).clamp(min=0.0)         # (B,K)
                weight = (cos_harm - allow_max).clamp(min=0.0)         # (B,K)
                weight = weight * cond_soft.float()                    # 조건 위치만
                soft_w = weight[:, None, :].expand(B, Q, K).repeat_interleave(Hh, dim=0)
                if soft_w.any():
                    scores = scores - (soft_w * self.cfg.gamma)
            ...
            if self.cfg.dual:
                ...
                # legacy: treat τ as hard threshold (allow 면제 동일 적용)
                over_hard = (cos_harm >= self.cfg.tau)
                if safe_mask is not None:
                    over_hard = over_hard & (~safe_mask)

            if over_hard is not None and over_hard.any():
                hard_exp = over_hard[:, None, :].expand(B, Q, K).repeat_interleave(Hh, dim=0)
...
def main(model=None):
    ...
    for idx, prompt in enumerate(prompts):
        ...
        # ★ SOT soft-delete 제외 마스크 주입
        if args.harm_suppress and (harm_proc is not None):
