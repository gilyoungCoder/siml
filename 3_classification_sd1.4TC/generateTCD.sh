...
OUTPUT_DIR="Continual/debug_pure0.05,0.1/i2p/addclothed__40,15_tau-0.5_rel-alpha0.2_EOTblock_true"
...
HARM_LAYER_INDEX=-2
HARM_TAU=0.20
HARM_REL_PARAM=0.05      # alpha if RELATIVE_MODE=alpha, tau_rel if RELATIVE_MODE=ratio
HARM_GAMMA_START=40
HARM_GAMMA_END=0.5
...
# =========================
DEBUG=true
ATTN_DEBUG_EVERY=10        # N스텝마다 1회만 출력 예산 지급
ATTN_DEBUG_TOPK=10          # Top-|Δ| K개
ATTN_DEBUG_THRESHOLD=0.01  # mean|Δ| < T 이면 스킵
ATTN_DEBUG_SLICE=true     # b0,h0,q0 슬라이스 1줄

# =========================
# Output & Log
# =========================
mkdir -p "${OUTPUT_DIR}"
LOG="./DEBUG_$(date +%Y%m%d_%H%M%S).log"
...
if [ "${DEBUG}" = true ]; then
  ...
  DBG_ARGS+=("--attn_debug_every" "${ATTN_DEBUG_EVERY}")
  DBG_ARGS+=("--attn_debug_topk" "${ATTN_DEBUG_TOPK}")
  DBG_ARGS+=("--attn_debug_threshold" "${ATTN_DEBUG_THRESHOLD}")
  if [ "${ATTN_DEBUG_SLICE}" = true ]; then
    DBG_ARGS+=("--attn_debug_slice")
  fi
fi
