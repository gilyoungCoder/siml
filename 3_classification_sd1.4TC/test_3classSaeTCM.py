...
# =========================
def parse_args():
    parser = ArgumentParser(description="Generation script (with global suppression + allow + freedom + SAE)")
    ...
    parser.add_argument("--harm_tau", type=float, default=0.10, help="Cosine threshold τ")
    ...
    parser.add_argument("--harm_hard_block", action="store_true", help="Hard block (mask -1e9) instead of subtracting γ")
...
# =========================
class HarmCfg:
    __slots__ = ("enable", "tau", "gamma", "allow_tau", "hard_block")

    def __init__(self, enable=True, tau=0.1, gamma=1.0, allow_tau=0.35, hard_block=False):
...
class HarmfulAttnProcessor(AttnProcessor2_0):
    ...
    - register_buffer 미사용 (버전 독립).
    - 생성자에서 device 요구 X, 호출 시점에 디바이스 맞춤.
    - Dropout이 모듈/스칼라 모두 안전 처리.
    """
    def __init__(
        self,
        harm_vec: Optional[torch.Tensor],
        ...
        super().__init__()
        self.cfg = cfg
        self.training = False  # 일부 diffusers 버전에서 dropout 경로에 필요할 수 있음
    ...
    def __call__(
        ...
        if is_cross and self.cfg.enable and (harm_vec is not None):
            ...
            over_expanded = over[:, None, :].expand(B, Q, K)            # (B, Q, K)
            over_expanded = over_expanded.repeat_interleave(H, dim=0)   # (B*H, Q, K)

            if self.cfg.hard_block:
                scores = scores.masked_fill(over_expanded, -1e9)
            else:
                scores = scores - (over_expanded.float() * self.cfg.gamma)
...
def main(model=None):
        ...
        if len(global_concepts) == 0:
            ...
            harm_proc = HarmfulAttnProcessor(
                ...
                cfg=HarmCfg(
                    ...
                    hard_block=args.harm_hard_block,
                ),
            )
    ...
    for idx, prompt in enumerate(prompts):
        ...
        # optional debug print of token-wise cosines before generation
        ...
        # save first image of this prompt
