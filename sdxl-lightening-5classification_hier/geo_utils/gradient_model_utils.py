...
class TimeDependentDiscriminatorGradientModel(DiscriminatorGradientModel):
    ...
    def get_differentiate_value(self, image, timestep, encoder_hidden_states=None, target_class: int = 1):
        # 1) timestep을 배치 크기에 맞춰 복제
        ts = torch.tile(timestep, (image.shape[0],))

        # 2) 4‑head 계층형 분류기로부터 로짓 얻기
        out0, out1, outc, outn = self.model(image, ts)

        # 3) softmax 확률로 변환
        p0 = F.softmax(out0, dim=1)  # [B,2]  → p0[:,0]=non-people, p0[:,1]=people
        p1 = F.softmax(out1, dim=1)  # [B,2]  → p1[:,0]=clothed,    p1[:,1]=nude
        ...
        # 6) 안전도 지표: log(p_safe / p_unsafe)
        log_ratio = torch.log(p_safe) - torch.log(p_unsafe)  # [B]

        # 7) 배치 전체 스칼라 합계
        diff_val = log_ratio.sum()

        # 8) 모니터링 용도로 safe 확률만 반환
