...
class TimeDependentDiscriminatorGradientModel(DiscriminatorGradientModel):
    def __init__(self, model_config_obj, device="cpu"):
        super().__init__(model_config_obj, device)
        ##NAG
        self.phi       = 5.0  # guidance scale ϕ
        self.tau       = 2.5  # norm clip τ
        self.alpha     = 0.5  # blending α

    def get_scaled_input(self, diffusion_pipeline, noisy_input, noise_pred, timestep):
        return get_noisy_xt(diffusion_pipeline, noisy_input, noise_pred, timestep)

    # def get_differentiate_value(self, image, timestep, encoder_hidden_states=None, target_class: int = None):
        # # 1) logits 얻기
        # B = image.shape[0]
        # t = torch.tile(timestep, (B, ))
        # logits = self.model(image, t)            # [B, C]
        # tc = [1]
        # nc = [10]

        # # 2) positive/negative 로짓 분리
        # f_pos = logits[:, tc]                   # [B]
        # f_neg = logits[:, nc]                   # [B]

        # # 3) Extrapolation: Ze = f_pos + ϕ·(f_pos − f_neg)
        # Ze = f_pos + self.phi * (f_pos - f_neg)  # [B]

        # # 4) L1‐norm 기반 정규화 & τ 클리핑
        # eps      = 1e-6
        # norm_pos   = f_pos.abs()   + eps       # [B]
        # norm_tilde = Ze.abs()      + eps       # [B]
        # R = norm_tilde / norm_pos                # [B]
        # R_clipped = torch.clamp(R, max=self.tau) # [B]
        # Zb = Ze * (R_clipped / R)               # [B]

        # # 5) Blending: Z_nag = α·Zb + (1−α)·f_pos
        # Z_nag = self.alpha * Zb + (1 - self.alpha) * f_pos  # [B]

        # # 6) 최종 differentiate_value
        # diff_val       = Z_nag.sum()           # scalar
        # output_for_log = Z_nag.detach()        # [B]
        # return diff_val, output_for_log
    def get_differentiate_value(self, image, timestep, encoder_hidden_states=None, target_class: int = 1):
        ...
        probs = F.softmax(logits, dim=-1)[:, 10] #target class=1
        ...
        # safe_classes=[1]  # fully_clothed부터 lingerie까지
        # unsafe_classes=[10]     
        # # (1) timestep 배치 크기 맞춰 확장
        # timestep = torch.tile(timestep, (image.shape[0], ))

        # # (2) 11-way logits → softmax 확률
        # logits = self.model(image, timestep)         # [B,11]
        # probs  = F.softmax(logits, dim=-1)           # [B,11]

        # # (3) 안전/불안전 확률 합산
        # p_safe   = probs[:, safe_classes].sum(dim=1)   # [B]
        # p_unsafe = probs[:, unsafe_classes].sum(dim=1) # [B]

        # # (4) 수치 안정성 클램핑
        # p_safe   = torch.clamp(p_safe,   min=1e-5, max=1.0)
        # p_unsafe = torch.clamp(p_unsafe, min=1e-5, max=1.0)

        # # (5) 안전도 지표: log(p_safe / p_unsafe)
        # log_ratio = torch.log(p_safe) - torch.log(p_unsafe) # [B]

        # # (6) 배치 전체 스칼라
        # diff_val = log_ratio.sum()       # scalar

        # # (7) 모니터링 용도로 안전 확률만 리턴
        # output_for_log = p_safe.detach() # [B]

        # return diff_val, output_for_log
        
        # safe_classes = [1, 2, 3]
        # unsafe_classes = [8, 9, 10]

        # # (1) timestep 확장
        # timestep = torch.tile(timestep, (image.shape[0],))

        # # (2) logits → [B, 11]
        # logits = self.model(image, timestep)

        # # (3) 각 클래스별 확률
        # probs = F.softmax(logits, dim=-1)  # [B, 11]

        # # (4) 각 클래스별 one-hot vector 생성 (projection axis)
        # device = logits.device
        # class_vectors = torch.eye(logits.shape[1], device=device)  # [11, 11]

        # # (5) safe/unsafe 클래스의 벡터 추출
        # safe_vecs   = class_vectors[safe_classes]   # [3, 11]
        # unsafe_vecs = class_vectors[unsafe_classes] # [3, 11]

        # # (6) 확률 벡터를 safe/unsafe 방향으로 projection
        # #     proj(p, v) = (p·v)v / (v·v) = (p·v)v  (v가 one-hot이면 v·v = 1)
        # p_safe_proj = torch.matmul(probs, safe_vecs.T)  # [B, 3]
        # p_unsafe_proj = torch.matmul(probs, unsafe_vecs.T)  # [B, 3]

        # # (7) norm 없이 단순합산: 간섭 줄이는 목적이므로 직접적인 scalar합보다 projection된 성분만 합산
        # p_safe_sum = p_safe_proj.sum(dim=1)   # [B]
        # p_unsafe_sum = p_unsafe_proj.sum(dim=1)  # [B]

        # # (8) 수치 안정화
        # p_safe_sum   = torch.clamp(p_safe_sum,   min=1e-5, max=1.0)
        # p_unsafe_sum = torch.clamp(p_unsafe_sum, min=1e-5, max=1.0)

        # # (9) log ratio
        # log_ratio = torch.log(p_safe_sum) - torch.log(p_unsafe_sum)  # [B]

        # # (10) 최종 guidance 값
        # diff_val = log_ratio.sum()  # scalar
        # output_for_log = p_safe_sum.detach()  # [B]
...
class LatentCLIPGradientModel(GradientModel):
    def __init__(self, model_args, device="cpu"):
        """
        model_args 예시:
          {
            "latent2clip_ckpt": "/path/to/latent2clip_epoch10.pt",
            "clip_text_encoder": "openai/clip-vit-large-patch14",
            "prompt": "a fully clothed person"
          }
        """
        super().__init__()
        self.device = device

        # 1) latent→CLIP 매핑 네트워크
        from train_latent2clip import Latent2Clip
        self.net = Latent2Clip(latent_dim=4, clip_dim=768).to(device).eval()
        self.net.load_state_dict(
            torch.load(model_args["latent2clip_ckpt"], map_location=device)
        )

        # 2) CLIP text encoder
        self.tokenizer = CLIPTokenizer.from_pretrained(model_args["clip_text_encoder"])
        self.text_encoder = CLIPTextModel.from_pretrained(model_args["clip_text_encoder"])\
                                          .to(device).eval()

        prompts = [model_args["prompt"]]
        tokens  = self.tokenizer(prompts, return_tensors="pt").to(device)
        with torch.no_grad():
            feats = self.text_encoder(**tokens).pooler_output  # (1, dim)
            self.txt_feats = feats / feats.norm(dim=-1, keepdim=True)
    ...
    def create_model(self):
        # 이미 init 에서 구성했으므로 빈 구현
        pass

    def load_model(self, *args, **kwargs):
        # 이미 init 에서 불러왔으므로 빈 구현
        pass

    def get_scaled_input(self, diffusion_pipeline, noisy_input, noise_pred, timestep):
        # x0 예측 (DDIM 식)
        #1) 타임스텝에 해당하는 α_t 가져오기
        alpha_t = diffusion_pipeline.scheduler.alphas_cumprod[timestep].to(noisy_input.device)
        # 2) sqrt(α_t) 와 sqrt(1 - α_t)
        sqrt_alpha_t = alpha_t.sqrt()
        sqrt_one_minus_alpha_t = (1 - alpha_t).sqrt()
        # 3) DDIM 식으로 x0 예측
        x0_pred = (noisy_input - sqrt_one_minus_alpha_t * noise_pred) / sqrt_alpha_t
        return x0_pred

    def get_differentiate_value(self, z, timestep=None, encoder_hidden_states=None, target_class=None):
        # z: (B,4,H,W) latent x0
        # 1) latent→CLIP
        out = self.net(z)                       # (B,768)
        out = out / out.norm(dim=-1, keepdim=True)
        # 2) cosine similarity
        sim = (out * self.txt_feats).sum(-1)    # (B,)
        return sim.sum(), sim.detach()

    def guide_samples(self, diffusion_pipeline, noise_pred, prev_latents, latents, timestep, grad, scale):
        # score-based diffusion + grad 적용
        alpha_cp = diffusion_pipeline.scheduler.alphas_cumprod[timestep].to(latents.device)
        denom    = (1 - alpha_cp).sqrt()
        score    = - noise_pred / denom
        adjusted = score + grad.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * scale
        new_noise = - adjusted * denom
        out = diffusion_pipeline.scheduler.step(
            new_noise, timestep, prev_latents, return_dict=True
        )
