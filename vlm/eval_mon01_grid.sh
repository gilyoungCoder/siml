#!/bin/bash
# Eval mon01_grid results (ringabell, unlearndiff, mma) with Qwen VLM
# Usage: CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 bash vlm/eval_mon01_grid.sh --nohup
...
if [ "$NOHUP" = true ]; then
    LOGFILE="results/eval_mon01_grid_$(date +%Y%m%d_%H%M%S).log"
    mkdir -p results
...
# Collect dirs needing eval (ringabell, unlearndiff, mma only)
DIRS=()
for ds in ringabell unlearndiff mma; do
    for config_dir in "$BASE_DIR/$ds"/*/; do
        [ -d "$config_dir" ] || continue
        IMG_COUNT=$(ls "$config_dir"/*.png 2>/dev/null | wc -l)
        [ "$IMG_COUNT" -gt 0 ] || continue
        [ -f "$config_dir/categories_qwen3_vl_nudity.json" ] && continue
        DIRS+=("$config_dir")
    done
done

TOTAL=${#DIRS[@]}
echo "=== mon01_grid folders to eval: $TOTAL ==="
