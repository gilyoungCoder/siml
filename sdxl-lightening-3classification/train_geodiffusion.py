...
# def set_model():
def collate_fn_foreground_loss(examples):
...
def collate_fn_non_foreground_loss(examples):
    pixel_values = torch.stack([example["pixel_values"] for example in examples])
    pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()
    input_ids = torch.stack([example["input_ids"] for example in examples]).to(memory_format=torch.contiguous_format).long()
...
def main(models=None):
    ...
    if args.num_bucket_per_side is not None:
        ...
        if num_vocabs != num_embeds:
            accelerator.print('Expanding text encoder embedding size from {} to {}'.format(num_embeds, num_vocabs))
            text_encoder.resize_token_embeddings(num_vocabs)
            if args.bucket_sincos_embed:
                accelerator.print('Use 2D sine-cosine embeddings to initialize bucket location tokens')
                from geo_utils.misc import get_2d_sincos_pos_embed
    ...
    # Select trainable parameter between lora and unet
    if args.lora_mode:
    ...
    unet_trainable_params = [param for param in unet.parameters() if param.requires_grad]
    ...
    params_to_optimize = misc.param_groups_lrd(
        ...
        layer_decay=args.lr_text_layer_decay,
        verbose=accelerator.is_local_main_process
    ) + [{'params': unet_trainable_params, 'name': 'unet'}] if args.train_text_encoder else unet_trainable_params
    ...
    if "new_coco_stuff" in args.dataset_config_name:
        train_dataset = NewCOCOStuffDataset(
                **dataset_cfg.data.train, blip_finetune=not args.no_blip_finetune, 
                tokenizer=tokenizer, is_main_process=accelerator.is_local_main_process)
    elif "missing_person" in args.dataset_config_name:
        train_dataset = MissingPersonDataset(
                **dataset_cfg.data.train, blip_finetune=not args.no_blip_finetune, 
                tokenizer=tokenizer, is_main_process=accelerator.is_local_main_process)
    else:
    ...
    if args.foreground_loss_mode:
        collate_fn = collate_fn_foreground_loss
    else:
        collate_fn = collate_fn_non_foreground_loss

    num_workers = 4
    if os.environ.get("PJRT_DEVICE", None) == "TPU":
        # num_workers = 4 * torch_xla.device_count()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # accelerator.print("before gather") # This takes a lot time
            ...
            if global_step % args.save_ckpt_freq == 0:
                ...
                if args.lora_mode:
                    ...
                    unet_module_cpu = copy.deepcopy(unet_module).cpu()
                    unet_lora_state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(unet_module_cpu))
                    lora_dir = os.path.join(args.output_dir, 'checkpoint', 'iter_' + str(global_step), "lora")
                    ...
                    text_encoder_dir = os.path.join(args.output_dir, 'checkpoint', 'iter_' + str(global_step), "text_encoder")
                    os.makedirs(text_encoder_dir, exist_ok=True)
                    
                    unwrapped_text_encoder = accelerator.unwrap_model(text_encoder)
                    text_encoder_dir = os.path.join(text_encoder_dir, "model.pth")
                    text_encoder_cpu = copy.deepcopy(unwrapped_text_encoder).to("cpu")
                    torch.save(text_encoder_cpu.state_dict(), text_encoder_dir)
                    ...
                    save_pipeline = copy.deepcopy(pipeline).to("cpu")
                    save_pipeline.save_pretrained(
                        os.path.join(args.output_dir, 'checkpoint', 'iter_' + str(global_step)),
                        is_main_process=accelerator.is_local_main_process,)
