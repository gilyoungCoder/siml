...
# =========================
# Machine Unlearning with ADAPTIVE THRESHOLD (2-CLASS VERSION)
# SAFREE-style dynamic threshold per prompt
# For classifier with 2 output channels (benign vs harmful)
# =========================
...
# =========================
# Classifier Guidance (2-CLASS VERSION)
# =========================
CLASSIFIER_GUIDANCE=true
CLASSIFIER_CONFIG="./configs/models/time_dependent_discriminator2class.yaml"
CLASSIFIER_CKPT="./work_dirs/nudity_two_class_latest/checkpoint/step_6200/classifier.pth"  # ← 2-class checkpoint
GUIDANCE_SCALE=7.5
GUIDANCE_START_STEP=1
TARGET_CLASS=0  # ← 2-class: 0=benign/safe, 1=harmful (nudity)
...
# =========================
mkdir -p "${OUTPUT_DIR}"
LOG="./logs/run_adaptive_2class_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "${LOG}")"

echo ""
echo "╔════════════════════════════════════════════════════════════════════════════════╗"
echo "║         MACHINE UNLEARNING - ADAPTIVE THRESHOLD MODE (2-CLASS)                 ║"
echo "╚════════════════════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "└───────────────────────────────────────────────────────────────────────────────┘"
echo ""
echo "┌─ Classifier Guidance (2-CLASS) ───────────────────────────────────────────────┐"
echo "│  Enabled: ${CLASSIFIER_GUIDANCE}"
echo "│  Checkpoint: ${CLASSIFIER_CKPT}"
echo "│  Scale: ${GUIDANCE_SCALE}"
echo "│  Target class: ${TARGET_CLASS} (benign/safe)"
echo "│  Class mapping: 0=benign/safe, 1=harmful/nudity"
...
echo "[INFO] Starting generation with ADAPTIVE THRESHOLD mode (2-CLASS)..."
