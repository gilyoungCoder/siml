#!/bin/bash
# ============================================================================
# Always-On Adaptive Spatial CG - Test Run Script
# 감지 단계 없이 항상 적용, adaptive spatial threshold만 사용
# ============================================================================
...
# OUTPUT_BASE_DIR="./scg_outputs/always_adaptive_spatial_cg/vangogh"
CKPT_PATH="CompVis/stable-diffusion-v1-4"
# CLASSIFIER_PATH="./work_dirs/nudity_three_class/checkpoint/step_11800/classifier.pth"
# OUTPUT_BASE_DIR="./scg_outputs/always_adaptive_spatial_cg/nudity"
CLASSIFIER_PATH="./work_dirs/jtt_stage2_T5800/checkpoints/step_18400/classifier.pth"
OUTPUT_BASE_DIR="./scg_outputs/nudity_jtt_step18400_re"
# ============================================================================
# 기본 생성 파라미터
# ============================================================================
...
# ============================================================================
# Adaptive Spatial CG 파라미터 (핵심 4개만!)
# ============================================================================

# Absolute Normalization 기반 설정
# GradCAM 통계: mean=0.0118, std=0.0149
# Z-score 변환 후 Gaussian CDF → [0, 1] 확률값
#
# 확률 해석 (training data 기준):
#   0.93 (p90, z=+1.5σ) = 상위 7% (강한 유해 신호)
#   0.70 (p75, z=+0.5σ) = 상위 30% (중간 유해 신호)
#   0.50 (평균)          = 중간값
#   0.30 (z=-0.5σ)      = 하위 30%

GUIDANCE_SCALE=10.0                   # 가이던스 강도 (기존 15→10으로 약화)
SPATIAL_THRESHOLD_START=0.45          # 초기: 상위 25% 영역만 (보수적 시작)
SPATIAL_THRESHOLD_END=0.15            # 최종: 평균 이상 영역 (균형)
THRESHOLD_STRATEGY="cosine_anneal"    # cosine_anneal (초반/후반 완만, 중간 급격)

# 선택적 파라미터
USE_BIDIRECTIONAL="--use_bidirectional"  # 양방향 가이던스 활성화
HARMFUL_SCALE=1.5                        # 유해 반발력 (기존 1.5→1.2로 약화)
...
# GradCAM Statistics (Absolute Normalization)
GRADCAM_STATS_FILE="./gradcam_nudity_stats_jtt_step18400.json"  # ← 추가!
...
main() {
    print_header "Always-On Adaptive Spatial CG - 실험 시작"
