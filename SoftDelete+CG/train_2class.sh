...
# (benign / nudity 2개 클래스로 DDPM 노이즈 주입 학습)
#───────────────────────────────────────────────────────────────────────────────#
...
# (2) 학습에 사용할 경로 설정
BENIGN_DIR=/mnt/home/yhgil99/dataset/threeclassImg/clothed5k        # 사람 없음
...
OUTPUT_DIR=work_dirs/nudity_two_class_latest   # 체크포인트·로그 저장 폴더
...
USE_WANDB="--use_wandb --report_to wandb --wandb_project two_class_project --wandb_run_name twoclass_run"
...
nohup python train_2class.py \
  ...
  $USE_WANDB \
  > trainClass_2class.log 2>&1 &

echo "Training launched. Logs are in trainClass_2class.log"
