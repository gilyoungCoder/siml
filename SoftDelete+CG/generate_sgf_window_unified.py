...
# -*- coding: utf-8 -*-
"""
SGF Time Window + ASCG Spatial CG (Unified).

Combines:
  - SGF's "when": Critical time window from CBF theory (guidance only in t=[end_t, start_t])
  - SGF's "how much": Time-varying guidance strength λ(t) that decays within the window
  - ASCG's "where": Spatial GradCAM masking (CDF-normalized binary mask)
  - ASCG's "how": Dual classifier gradient g = ∇log p(safe) - ∇log p(harm)

Monitoring modes (legacy) are replaced by timestep-based window control.
"""
...
def load_gradcam_stats(stats_dir: str) -> Dict:
    """Load topk (for spatial) and sample-level statistics."""
...
# =============================================================================
# SGF Time Window: "When" to apply guidance
# =============================================================================
def sgf_should_guide(timestep_value: int, start_t: int, end_t: int) -> bool:
    """SGF critical window: guide only when end_t <= t <= start_t."""
    return end_t <= timestep_value <= start_t


def sgf_guidance_weight(timestep_value: int, start_t: int, end_t: int,
                        schedule: str = "cosine") -> float:
    """
    SGF-inspired guidance strength schedule within the critical window.

    Args:
        timestep_value: current timestep (e.g., 981, 961, ...)
        start_t: window start (e.g., 1000)
        end_t: window end (e.g., 800)
        schedule: "constant", "cosine", "linear", "sigmoid"

    Returns:
        weight in [0, 1] that scales the guidance strength
    """
    if not sgf_should_guide(timestep_value, start_t, end_t):
        return 0.0
    ...
    # Progress within window: 0 = start (early), 1 = end (late)
    progress = (start_t - timestep_value) / window_size
    ...
    if schedule == "constant":
        ...
        # Sigmoid: sharp transition centered at midpoint
        # Maps progress [0,1] -> weight [~1, ~0] via sigmoid(-10*(x-0.5))
...
# =============================================================================
# Spatial Guidance (same as generate_unified_monitoring.py)
# =============================================================================
class SpatialGuidance:
    ...
    def compute_gradient(self, latent, timestep, active_harm_classes, spatial_threshold,
        ...
        if spatial_mode == "gradcam":
            ...
            attn_mask = self._compute_attention_mask(0.0)
            if attn_mask is not None:
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default="./scg_outputs/sgf_window_unified")
    ...
    # === SGF Time Window (replaces monitoring) ===
    ...
    parser.add_argument("--guidance_schedule", type=str, default="cosine",
                        choices=["constant", "cosine", "linear", "sigmoid"],
                        help="How guidance strength varies within the window")

    # Spatial guidance mode (ASCG "where")
    ...
    # Guidance scales (ASCG "how much")
    parser.add_argument("--guidance_scale", type=float, default=10.0,
                        help="Peak guidance scale (multiplied by SGF window weight)")
    parser.add_argument("--base_guidance_scale", type=float, default=2.0,
                        help="Guidance scale outside spatial mask region")
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"SGF TIME WINDOW + ASCG SPATIAL CG")
    ...
    for prompt_idx, prompt in tqdm(prompts_with_idx, desc="Generating"):
        ...
        for sample_idx in range(args.nsamples):
            ...
            for step_idx, t in enumerate(scheduler.timesteps):
                ...
                # 4. SGF Time Window Check + Spatial CG
                in_window = sgf_should_guide(t_val, args.guidance_start_t, args.guidance_end_t)
                window_weight = sgf_guidance_weight(
                    t_val, args.guidance_start_t, args.guidance_end_t,
                ...
                if in_window and window_weight > 0.01:
                    # Active harm classes: always [2, 3] within the window
                    # (SGF assumes all steps in window need guidance)
                    active_classes = [2, 3]
                    ...
                    # Scale guidance by SGF window weight
                    effective_scale = args.guidance_scale * window_weight
                    effective_base = args.base_guidance_scale * window_weight
                    ...
                    info["guided"] = True
                    info["effective_scale"] = effective_scale

                    if args.debug and step_idx % 5 == 0:
                        print(f"  Step {step_idx}: t={t_val}, weight={window_weight:.3f}, "
                              f"scale={effective_scale:.1f}, spatial_thr={spatial_thr:.3f}")
                else:
                    skipped_steps_count += 1
                    info["guided"] = False

                    if args.debug and step_idx % 10 == 0:
                        print(f"  Step {step_idx}: t={t_val} [outside window]")
