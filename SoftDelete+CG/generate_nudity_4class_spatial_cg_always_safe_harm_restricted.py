...
# -*- coding: utf-8 -*-
"""
Nudity 4-Class Adaptive Spatial Classifier Guidance (SAFE-HARM RESTRICTED VERSION)
...
Guidance Logic (SAFE-HARM RESTRICTED version):
  At each step:
  1. Compute classifier logits for current latent
  2. ALWAYS select max among harm classes (2, 3)
  3. Apply RESTRICTED GRADIENT between safe and harm:
...
Key Features:
  - Restricted gradient between safe and harm (removes conflicting directions)
  - This prevents safe gradient from accidentally reinforcing harm direction
  - And prevents harm gradient from accidentally reinforcing safe direction
...
# =========================
# Selective Spatial Guidance with Safe-Harm Restricted Gradient
# =========================
class SelectiveSpatialGuidance:
    """
    Applies RESTRICTED GRADIENT between safe and harm gradients.
    Removes conflicting directions to prevent interference.
    ...
    def _project_out(self, g_a: torch.Tensor, g_b: torch.Tensor) -> torch.Tensor:
        ...
        delta_a = g_a - (g_a^T g_b / ||g_b||^2) * g_b
        """
        g_a_flat = g_a.view(-1)
        g_b_flat = g_b.view(-1)
    ...
    def compute_gradient(
        ...
        """
        Compute spatially-masked RESTRICTED gradient between safe and harm.
        ...
        with torch.enable_grad():
            ...
            if self.use_bidirectional:
                ...
                # delta_safe = g_safe - proj(g_safe onto g_harm)
                # delta_harm = g_harm - proj(g_harm onto g_safe)
                delta_safe = self._project_out(grad_safe, grad_harmful)
                delta_harm = self._project_out(grad_harmful, grad_safe)

                # Final: delta_safe - harmful_scale * delta_harm
                grad = delta_safe - harmful_scale * delta_harm
            else:
                # Unidirectional: pull to safe only (no restricted needed)
