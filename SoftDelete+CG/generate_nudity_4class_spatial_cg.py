...
# -*- coding: utf-8 -*-
"""
Nudity 4-Class Adaptive Spatial Classifier Guidance
...
Guidance Logic:
  ...
  2. Find max logit among all classes
  3. If max class is 2 (harm_nude) or 3 (harm_color):
     - Compute GradCAM heatmap for the detected harm class
     - Create spatial mask using adaptive threshold
     - Apply bidirectional guidance: -(harm_grad - safe_grad) = safe_grad - harm_grad
  4. If max class is 0 (benign) or 1 (safe_clothed):
     - Skip guidance (no intervention needed)

Key Features:
  - Selective guidance: only when harmful content detected
...
# =========================
# Harm Class Detector (4-class)
# =========================
class HarmClassDetector:
    """
    Detects if current latent shows harmful content (harm_nude or harm_color).
    Returns the detected harm class for targeted guidance.
    """
    ...
    def detect_harm(
        ...
        Returns:
            is_harmful: Whether max logit is a harm class
            harm_class: Index of detected harm class (None if safe)
...
def load_prompts(prompt_file: str) -> List[str]:
    if prompt_file.endswith('.csv'):
        ...
        with open(prompt_file, 'r') as f:
            ...
            prompts = [row['prompt'] for row in reader if row.get('prompt', '').strip()]
    else:
        with open(prompt_file, 'r') as f:
...
def generate_with_selective_guidance(
    ...
    for prompt_idx, prompt in enumerate(tqdm(prompts, desc="Prompts")):
        ...
        for sample_idx in range(args.nsamples):
            ...
            early_exit_flag = [False]  # mutable for closure
            ...
            def callback_on_step_end(pipe, step, timestep, callback_kwargs):
                ...
                if args.guidance_start_step <= step <= args.guidance_end_step:
                    ...
                    # Early exit: if step 0 max class is safe (benign=0 or safe_clothed=1), skip all remaining guidance
                    if step == args.guidance_start_step and detection_info['max_class'] in (0, 1):
                        early_exit_flag[0] = True
                        print(f"  [Early exit] Step {step} {detection_info['max_class_name']} -> guidance disabled")
                        return callback_kwargs
            ...
            stats = mask_generator.get_statistics()
            guided_steps = [h['step'] for h in stats['step_history'] if h['is_harmful']]
            print(f"  Guidance: {len(guided_steps)}/{stats['total_steps']} steps "
                  f"({len(guided_steps)/max(1,stats['total_steps'])*100:.0f}%)")
            if guided_steps:
                print(f"  Guided steps: {guided_steps}")
