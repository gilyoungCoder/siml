#!/bin/bash
# Dual_path rerun - 8 configs on 8 GPUs in parallel
# Configs 0,1 already done. This runs configs 2-9.
# Settings: ds=14, z0t=0.0005, cdf=0.05
...
OUTPUT_BASE="scg_outputs/dual_path_rerun"
...
run_job() {
    ...
    local EXP="dp_gradcam_gs${GS}_bs${BS}_sp${SP_S}-${SP_E}_ds14_z0t0.0005"
    local OUT="${OUTPUT_BASE}/ringabell/${EXP}"
    local LOG="/tmp/dualpath_${EXP}.log"
...
echo "All 8 dual_path jobs launched. Waiting..."
wait
echo "=== ALL DUAL_PATH DONE ==="
...
for job in "${JOBS[@]}"; do
    IFS='|' read -r GS BS SP_S SP_E GPU <<< "$job"
    EXP="dp_gradcam_gs${GS}_bs${BS}_sp${SP_S}-${SP_E}_ds14_z0t0.0005"
