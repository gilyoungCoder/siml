...
# =========================
# Machine Unlearning with ADAPTIVE THRESHOLD (2-CLASS VERSION - VIOLENCE)
# SAFREE-style dynamic threshold per prompt
# For classifier with 2 output channels (non-violent vs violent)
# =========================
...
OUTPUT_DIR="./SDCG/np/violence/adaptive_2class"
...
# =========================
# Classifier Guidance (2-CLASS VERSION - VIOLENCE)
# =========================
CLASSIFIER_GUIDANCE=true
CLASSIFIER_CONFIG="./configs/models/time_dependent_discriminator2class.yaml"
CLASSIFIER_CKPT="./work_dirs/violence_two_class/checkpoint/step_6700/classifier.pth"  # ← 2-class checkpoint
GUIDANCE_SCALE=7.5
GUIDANCE_START_STEP=1
TARGET_CLASS=0  # ← 2-class: 0=non-violent/safe, 1=violent
...
# =========================
mkdir -p "${OUTPUT_DIR}"
LOG="./logs/run_adaptive_violence_2class_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "${LOG}")"

echo ""
echo "╔════════════════════════════════════════════════════════════════════════════════╗"
echo "║      MACHINE UNLEARNING - ADAPTIVE THRESHOLD MODE (2-CLASS VIOLENCE)           ║"
echo "╚════════════════════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "└───────────────────────────────────────────────────────────────────────────────┘"
echo ""
echo "┌─ Classifier Guidance (2-CLASS VIOLENCE) ──────────────────────────────────────┐"
...
echo "│  Class mapping: 0=non-violent/safe, 1=violent"
...
echo "[INFO] Starting generation with ADAPTIVE THRESHOLD mode (2-CLASS VIOLENCE)..."
