#!/bin/bash
# ============================================================================
# Cluster-Aware Adaptive Spatial CG - Run Script
# Clustering + Spatial Guidance 결합 추론
# ============================================================================
...
CENTROIDS_PATH="./cluster_centroids/harmful_clusters.pt"  # 클러스터 센트로이드 경로
GRADCAM_STATS_FILE="./gradcam_nudity_stats_jtt_step18400.json"
OUTPUT_BASE_DIR="./scg_outputs/cluster_spatial_cg"
...
SPATIAL_THRESHOLD_START=0.45
SPATIAL_THRESHOLD_END=0.15
...
# 클러스터별 스케일 (선택적)
# 예: '{"0": 15.0, "1": 10.0, "2": 12.0}'
# 특정 클러스터에 더 강한 가이던스를 적용하고 싶을 때 사용
CLUSTER_SCALES=""
# CLUSTER_SCALES='--cluster_scales {"0": 15.0, "1": 8.0}'
...
main() {
    print_header "Cluster-Aware Adaptive Spatial CG"
...
if [ ! -f "$CENTROIDS_PATH" ]; then
    ...
    print_info "먼저 cluster_harmful_latents.py를 실행하여 센트로이드를 생성하세요:"
    echo ""
    echo "  python mode_aware_guidance/cluster_harmful_latents.py \\"
    echo "    --mode generate \\"
    echo "    --ckpt_path $CKPT_PATH \\"
    echo "    --classifier_ckpt $CLASSIFIER_PATH \\"
    echo "    --prompts_file ./prompts/sexual_50.txt \\"
    echo "    --output_path $CENTROIDS_PATH \\"
    echo "    --n_clusters 10 \\"
    echo "    --save_latents"
    echo ""
    exit 1
