#!/usr/bin/env bash
#───────────────────────────────────────────────────────────────────────────────#
# three_class 분류기 학습용 실행 스크립트 (Grayscale 버전)
# (benign / person / nudity 3개 클래스로 DDPM 노이즈 주입 학습)
# 모든 이미지를 흑백으로 변환하여 색상 정보 없이 형태만으로 분류
#───────────────────────────────────────────────────────────────────────────────#
...
OUTPUT_DIR=work_dirs/nudity_three_class_grayscale_variousimg   # 체크포인트·로그 저장 폴더

# (4) 하이퍼파라미터
BATCH_SIZE=32
LEARNING_RATE=1e-4
SAVE_FREQ=100                                           # 스텝마다 체크포인트 저장
MAX_EPOCHS=100                                           # --num_train_epochs

# (5) WandB 설정 (사용 안 하려면 --use_wandb, --report_to 옵션 제외)
USE_WANDB="--use_wandb --report_to wandb --wandb_project three_class_project --wandb_run_name threeclass_run_grayscale"
...
nohup python train_3class.py \
  ...
  --grayscale \
  $USE_WANDB \
  > trainClass_gray.log 2>&1 &

echo "Training launched (Grayscale mode). Logs are in trainClass_gray.log"
