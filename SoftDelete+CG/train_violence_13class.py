...
# Class mapping for 13-class violence classifier
# 0: benign (no people, landscapes, objects)
# 1-3: fighting triplet (harm, safe, artifact)
# 4-6: weapon triplet (harm, safe, artifact)
# 7-9: blood triplet (harm, safe, artifact)
# 10-12: war triplet (harm, safe, artifact)
...
# Harm classes: 1, 4, 7, 10
HARM_CLASSES = [1, 4, 7, 10]

# Safe classes: 2, 5, 8, 11
SAFE_CLASSES = [2, 5, 8, 11]

# Artifact classes: 3, 6, 9, 12
ARTIFACT_CLASSES = [3, 6, 9, 12]

# Harm -> Safe mapping for guidance
HARM_TO_SAFE = {
    1: 2,   # harm_fighting -> safe_fighting
    4: 5,   # harm_weapon -> safe_weapon
    7: 8,   # harm_blood -> safe_blood
    10: 11, # harm_war -> safe_war
}

# Harm -> Artifact mapping
HARM_TO_ARTIFACT = {
    1: 3,   # harm_fighting -> artifact_fighting
    4: 6,   # harm_weapon -> artifact_weapon
    7: 9,   # harm_blood -> artifact_blood
    10: 12, # harm_war -> artifact_war
}


class Violence13ClassDataset(Dataset):
    """
    13-class violence dataset:
      0: benign (no people)
    ...
    def __init__(self, class_dirs: Dict[int, str], transform=None, balance_classes=True, seed=None):
        ...
        for class_id in sorted(class_paths.keys()):
            print(f"  Class {class_id:2d} ({CLASS_NAMES.get(class_id, 'unknown'):20s}): {len(class_paths[class_id])} samples")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 13-class violence classifier with DDPM noise injection"
    )
    ...
    parser.add_argument("--artifact_fighting_dir", type=str, required=True, help="Directory for artifact_fighting (class 3)")

    # Weapon triplet
    parser.add_argument("--harm_weapon_dir", type=str, required=True, help="Directory for harm_weapon (class 4)")
    parser.add_argument("--safe_weapon_dir", type=str, required=True, help="Directory for safe_weapon (class 5)")
    parser.add_argument("--artifact_weapon_dir", type=str, required=True, help="Directory for artifact_weapon (class 6)")

    # Blood triplet
    parser.add_argument("--harm_blood_dir", type=str, required=True, help="Directory for harm_blood (class 7)")
    parser.add_argument("--safe_blood_dir", type=str, required=True, help="Directory for safe_blood (class 8)")
    parser.add_argument("--artifact_blood_dir", type=str, required=True, help="Directory for artifact_blood (class 9)")

    # War triplet
    parser.add_argument("--harm_war_dir", type=str, required=True, help="Directory for harm_war (class 10)")
    parser.add_argument("--safe_war_dir", type=str, required=True, help="Directory for safe_war (class 11)")
    parser.add_argument("--artifact_war_dir", type=str, required=True, help="Directory for artifact_war (class 12)")

    parser.add_argument("--output_dir", type=str, default="violence_13class_output")
    ...
    parser.add_argument("--wandb_project", type=str, default="violence_13class_project")
...
def main():
    ...
    # Build class directories dict (13 classes)
    class_dirs = {
        0: args.benign_dir,
        # Fighting triplet
        1: args.harm_fighting_dir,
        2: args.safe_fighting_dir,
        3: args.artifact_fighting_dir,
        # Weapon triplet
        4: args.harm_weapon_dir,
        5: args.safe_weapon_dir,
        6: args.artifact_weapon_dir,
        # Blood triplet
        7: args.harm_blood_dir,
        8: args.safe_blood_dir,
        9: args.artifact_blood_dir,
        # War triplet
        10: args.harm_war_dir,
        11: args.safe_war_dir,
        12: args.artifact_war_dir,
    }

    # Dataset & Dataloader
    full_dataset = Violence13ClassDataset(
    ...
    # Model: 13-way classifier
    classifier = load_discriminator(ckpt_path=None, condition=None, eval=False, channel=4, num_classes=13).to(device)
    ...
    while global_step < max_steps:
        for batch in train_loader:
            ...
            logits = classifier(noisy_latents, norm_ts)  # shape [B,13]
            ...
            if global_step % args.save_ckpt_freq == 0:
                ...
                if accelerator.is_local_main_process:
                    ...
                    with open(os.path.join(ckpt_dir, "class_mapping.yaml"), "w") as f:
                        yaml.dump({
                            ...
                            "harm_to_artifact": HARM_TO_ARTIFACT,
                        }, f)
                ...
                class_correct = {i: 0 for i in range(13)}
                class_total = {i: 0 for i in range(13)}
                ...
                for class_id in range(13):
                    if class_total[class_id] > 0:
                        ...
                        logger.info(f"  Class {class_id:2d} ({CLASS_NAMES[class_id]:20s}): {class_acc:.4f} ({class_correct[class_id]}/{class_total[class_id]})")
