...
# -*- coding: utf-8 -*-
"""
Nudity 4-Class Adaptive Spatial Classifier Guidance (NUDE ONLY VERSION)

4 classes:
  0: benign (no people)
  1: safe_clothed (person with clothes)
  2: harm_nude (nudity)
  3: harm_color (color artifacts/distortions) - IGNORED in this version

Guidance Logic (NUDE ONLY version):
  At each step:
  1. Compute classifier logits for current latent
  2. ALWAYS guide toward safe using harm_nude (class 2) ONLY
  3. harm_color (class 3) is completely ignored
  4. Compute GradCAM heatmap for harm_nude only
  5. Apply bidirectional guidance: safe_grad - harm_grad

Key Features:
  - Only uses harm_nude (class 2) for guidance
  - Ignores harm_color (class 3) completely
  - Tests if color artifacts are caused by harm_color guidance
...
NUDITY_4CLASS_CONFIG = {
    ...
    # Harm classes (guidance applied) - NUDE ONLY, ignore harm_color
    "harm_classes": [2],
