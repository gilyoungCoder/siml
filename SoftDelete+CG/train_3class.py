...
class ThreeClassDataset(Dataset):
    ...
    person_dirs can be a single directory (str) or multiple directories (list).
    If multiple directories, samples equally from each.
    """
    def __init__(self, benign_dir, person_dirs, nude_dir, transform=None, balance_classes=True, seed=None):
        # Collect all paths and labels per class
        benign_paths = [os.path.join(benign_dir, f) for f in sorted(os.listdir(benign_dir))]
        nude_paths = [os.path.join(nude_dir, f) for f in sorted(os.listdir(nude_dir))]
        ...
        if balance_classes:
            ...
            min_size = min(len(benign_paths), total_person, len(nude_paths))
            ...
            num_dirs = len(person_paths_per_dir)
            samples_per_dir = min_size // num_dirs
            remainder = min_size % num_dirs

            person_paths = []
            for i, paths in enumerate(person_paths_per_dir):
                ...
                person_paths.extend(sampled)
                print(f"    Dir {i+1}: sampled {n_samples} from {len(paths)} available")

            # If we still need more samples (due to small directories), sample more from larger ones
            if len(person_paths) < min_size:
                all_remaining = []
                for paths in person_paths_per_dir:
                    all_remaining.extend([p for p in paths if p not in person_paths])
                needed = min_size - len(person_paths)
                if len(all_remaining) >= needed:
                    person_paths.extend(random.sample(all_remaining, needed))

            print(f"[Dataset] Balanced class distribution:")
            print(f"  Class 0 (Benign): {len(benign_paths)} samples")
            print(f"  Class 1 (Person): {len(person_paths)} samples")
            print(f"  Class 2 (Nude):   {len(nude_paths)} samples")
            print(f"[Dataset] Total samples after balancing: {min_size * 3}\n")
        else:
            # No balancing: just combine all person paths
            person_paths = []
            for paths in person_paths_per_dir:
...
def parse_args():
    ...
    parser.add_argument("--person_data_path", type=str, nargs='+', required=True,
                        help="One or more directories for person (non-nude) images. If multiple, samples equally from each.")
...
def main():
    ...
    # Transforms (with balanced color augmentation to reduce color bias)
    transform_list = [
        transforms.ToTensor(),
    ...
    # Balanced color augmentation to prevent classifier from learning skin-tone = nude
    # BalancedColorAugmentation(),  # Custom moderate color randomization (DISABLED)
    # transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.15),  # (DISABLED)
    ...
    # clf_path="/mnt/home/yhgil99/unlearning/three_classificaiton_new/work_dirs/nudity_three_class_Imagenet/checkpoint/step_12600/classifier.pth"  # 자유 모델 체크포인트
    classifier = load_discriminator(ckpt_path=None, condition=None, eval=False, channel=4, num_classes=3).to(device)
    ...
    while global_step < max_steps:
        for batch in train_loader:
            ...
            logits = classifier(noisy_latents, norm_ts)  # shape [B,3]
            ...
            if global_step % args.save_ckpt_freq == 0:
                ...
                with torch.no_grad():
                    for vb in val_loader:
                        ...
                        vlogits = classifier(noisy_vlat, vnorm_ts)   # ← timestep 인자 사용하도록 수정됨
