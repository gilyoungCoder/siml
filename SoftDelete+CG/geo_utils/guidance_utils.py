...
from geo_utils.gradient_model_utils import LatentCLIPGradientModel
from mode_aware_guidance import ModeAwareTimeDependentDiscriminatorGradientModel
...
class GuidanceModel:
    ...
    def guidance(self, diffusion_pipeline, callback_kwargs, step, timestep, scale, target_class = None):
        with torch.enable_grad():
            ...
            # if isinstance(self.gradient_model, TimeDependentDiscriminatorGradientModel):
            #     print("projection")
            #     # 1) non-nude (class=1) 그래디언트 계산
            #     _, grad_non, _ = self.gradient_model.get_grad(
            #         scaled_latent,
            #         normalized_timestep,
            #         encoder_hidden_states,
            #         grad_input=prev_latents,
            #         target_class=1
            #     )

            #     # 2) nude (class=2) 그래디언트 계산
            #     _, grad_nude, _ = self.gradient_model.get_grad(
            #         scaled_latent,
            #         normalized_timestep,
            #         encoder_hidden_states,
            #         grad_input=prev_latents,
            ...
            #     # 3) Projection: grad_non에서 grad_nude 성분 제거
            #     B = grad_non.shape[0]
            #     flat_non  = grad_non.view(B, -1)
            #     flat_nude = grad_nude.view(B, -1)
            #     dot   = (flat_non * flat_nude).sum(dim=1, keepdim=True)                  # [B,1]
            #     norm2 = (flat_nude * flat_nude).sum(dim=1, keepdim=True).clamp(min=1e-8)  # [B,1]
            #     coeff = (dot / norm2).view(B, 1, 1, 1)                                    # [B,1,1,1]
            #     grad  = grad_non - coeff * grad_nude                                      # 투영된 그래디언트

            #     output_for_log = grad_non.norm()  # 필요하다면 grad_non.norm() 등으로 로그 가능
            # else:
            #     # 기존 경로: 단일 target_class 그래디언트
            #     differentiate_value, grad, output_for_log = self.gradient_model.get_grad(
            #         scaled_latent,
            #         normalized_timestep,
            #         encoder_hidden_states,
            #         grad_input=prev_latents,
            #         target_class=target_class if target_class is not None else self.target_class
            #     )
        grad = grad.detach()
        ...
        # Check if mode-aware model (needs step and x_t for mode detection)
        if isinstance(self.gradient_model, ModeAwareTimeDependentDiscriminatorGradientModel):
            latents = self.gradient_model.guide_samples(
                diffusion_pipeline, noise_pred, prev_latents, latents, timestep, grad, scale,
                step=step, x_t=scaled_latent.detach()
            )
        else:
        ...
        if hasattr(self.gradient_model, 'current_mode') and self.gradient_model.current_mode is not None:
            ...
            return_dict["mode_scale"] = self.gradient_model.current_mode_scale
