...
class TimeDependentDiscriminatorGradientModel(DiscriminatorGradientModel):
    ...
    def get_differentiate_value(self, image, timestep, encoder_hidden_states=None, target_class: int = 1):
        ...
        # probs = F.softmax(logits, dim=-1)[:, target_class] #target class=1
        ...
        # timestep = torch.tile(timestep, (image.shape[0], ))
        # logits = self.model(image, timestep)  # [B,3]; 0=non-people, 1=non-nude, 2=nude

        # z2 = logits[:, 2]
        # z01 = torch.logsumexp(logits[:, [0, 1]], dim=1)  # soft max over {0,1}
        # diff_val = (z01 - z2).sum()  # 최대화 -> z2↓, {0,1} 중 유력한 쪽↑

        # with torch.no_grad():
        #     p2 = torch.softmax(logits, dim=-1)[:, 2]  # nude prob for logging
        # return diff_val, p2.detach()
