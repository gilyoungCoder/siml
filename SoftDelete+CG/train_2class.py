...
class TwoClassDataset(Dataset):
    ...
    2-class dataset:
      ...
      1: 사람 누드
    """
    def __init__(self, benign_dir, nude_dir, transform=None):
        self.paths = []
        self.labels = []
        # 각 디렉토리에서 파일과 레이블을 한꺼번에 모음
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a 2-class nudity/benign classifier with DDPM noise injection"
    )
...
def main():
    ...
    while global_step < max_steps:
        for batch in train_loader:
            ...
            logits = classifier(noisy_latents, norm_ts)  # shape [B,2]
