#!/bin/bash
# ============================================================================
# Fine Grid Search: Monitoring-Only (4class) + Sticky Trigger
#
# Same as grid_search_fine_mon4class.sh but with --sticky_trigger:
#   Once P(harm) > threshold at any step → guide ALL remaining steps
#
# Parameters (same as original fine grid):
#   monitoring_threshold: [0.05, 0.1, 0.15]
#   guidance_scale: [12.5, 15, 17.5]
#   base_guidance_scale: [0.5, 1.0, 2.0]
#   spatial_threshold_pairs: 8
#   sticky_trigger: True (always on)
#
# Total: 216 configs per dataset × 3 datasets = 648
# Per site: 324 configs
#
# Usage:
#   bash grid_search_fine_mon4class_sticky.sh --site A          # first half
#   bash grid_search_fine_mon4class_sticky.sh --site B          # second half
#   bash grid_search_fine_mon4class_sticky.sh --site A --nohup  # background
#   bash grid_search_fine_mon4class_sticky.sh --dry-run         # print only
# ============================================================================
...
if [ -z "$SITE" ] && [ "$DRY_RUN" = false ]; then
    echo "Usage: bash grid_search_fine_mon4class_sticky.sh --site A|B [--nohup] [--dry-run]"
    exit 1
fi
...
if [ "$USE_NOHUP" = true ]; then
    ...
    LOG_DIR="./scg_outputs"
    mkdir -p "$LOG_DIR"
    LOG_FILE="${LOG_DIR}/fine_grid_mon4class_sticky_site${SITE}_${TIMESTAMP}.log"
...
# (same as original + sticky_trigger)
# ============================================
...
# Total per dataset: 3 x 3 x 3 x 8 = 216 (same as original, but output to different dir)

# VLM eval script
VLM_SCRIPT="/mnt/home/yhgil99/unlearning/vlm/opensource_vlm_i2p_all.py"
OUTPUT_BASE="/mnt/home/yhgil99/unlearning/SoftDelete+CG/scg_outputs/fine_grid_mon4class_sticky"
...
echo "=============================================="
echo "FINE GRID SEARCH: MONITORING (4class) + STICKY TRIGGER"
echo "=============================================="
