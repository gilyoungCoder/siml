...
class RepellencyMethod(ABC):
    def __init__(self, ref_data, embed_fn, n_embed, **kwargs):
        ...
        self.max_norm = kwargs.get('normalize', None)
...
class EuclideanRepellency(RepellencyMethod):
    def __init__(self, ref_data, embed_fn, num_timesteps, n_embed, **kwargs):
        super().__init__(ref_data, ref_data, embed_fn, num_timesteps, n_embed, **kwargs)
        self.scale = kwargs.get('scale', 1.0)
...
class RBFKernelRepellency(RepellencyMethod):
    def __init__(self, ref_data, embed_fn, num_timesteps, n_embed, **kwargs):
        super().__init__(ref_data, embed_fn, num_timesteps, n_embed, **kwargs)
        self.scale = kwargs.get('scale', 1.0)
...
class SparseRepellency(RepellencyMethod):
    ...
    def repellency_force(self, x_0_hat, **kwargs):
        # normalization (for sdv3)
        x_0_hat = x_0_hat / torch.norm(x_0_hat, dim=1, keepdim=True)
