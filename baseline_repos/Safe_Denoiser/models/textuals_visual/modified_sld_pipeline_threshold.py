...
class SafetyConfig(object):
    ...
    MEDIUM = {
        # 'negation_warmup_start' : 5,
        # 'negation_warmup_end' : 20,
        'negation_warmup_steps' : 20,
        "sld_warmup_steps": 10,
...
class ModifiedSLDPipeline_Rep(SLDPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
        ...
        super(ModifiedSLDPipeline_Rep, self).__init__(
            vae,
            text_encoder,
            tokenizer,
            unet,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        negation_warmup_step = kwargs.get("negation_warmup_steps", 40)
        negation_warmup_start = 0
        negation_warmup_end = 50
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor)):
            ...
            # repellency
            if (i >= negation_warmup_start) and (i <= negation_warmup_end): # Warmup
                if repellency_processor is not None:
                    ...
                    repellency_dict = repellency_processor.conditioning(x_0_hat, beta_threshold=True)
                    
                    if repellency_dict.get("is_negation", False):
                        # TO DEBUG
                        ...
                        noise = torch.randn(x_0_hat_repellenced.shape, 
                                            generator=generator, 
                                            device=x_0_hat_repellenced.device, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    pipe = ModifiedSLDPipeline_Rep.from_pretrained(
