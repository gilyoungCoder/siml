...
def mask_to_onp(input_embeddings, p_emb, masked_input_subspace_projection, concept_subspace_projection, 
                alpha=0., max_length=333, debug=False):
    """
    ie = [2, 77, 768] <-- pos + neg
    ms = [768, 768]
    cs = [768, 768]
...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def text_encoders_to_cpu(self):
        self.text_encoder.to("cpu")
        if self.text_encoder_2 is not None: self.text_encoder_2.to("cpu")
        if self.text_encoder_3 is not None: self.text_encoder_3.to("cpu")
        self.vae_transformer_to_device("cuda")
        torch.cuda.empty_cache()     
        
    def text_encoders_to_device(self, device: Union[str, torch.device]):
        self.text_encoder.to(device)
        if self.text_encoder_2 is not None: self.text_encoder_2.to(device)
        if self.text_encoder_3 is not None: self.text_encoder_3.to(device)
        torch.cuda.empty_cache()     
        
    def vae_transformer_to_cpu(self):
        self.vae.to("cpu")
        self.transformer.to("cpu")
        self.text_encoders_to_device("cuda")
        torch.cuda.empty_cache()     
        
    def vae_transformer_to_device(self, device: Union[str, torch.device]):
        self.vae.to(device)
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # print('clip_prompt_embeds', clip_prompt_embeds.shape) # [1, 77, 4096])
            # print('t5_prompt_embed', t5_prompt_embed.shape)


            prompt_embeds = torch.cat([clip_prompt_embeds, t5_prompt_embed], dim=-2)
            # print('prompt_embeds here', prompt_embeds.shape)
    ...
    def _new_encode_negative_prompt2(self, negative_prompt2, max_length, num_images_per_prompt, pooler_output=True, clip_model_index=0):
        ...
        uncond_embeddings = self.text_encoder_3(
            uncond_input.input_ids.to(device),
            attention_mask=uncond_input.attention_mask.to(device),
        )
    ...
    def _masked_encode_prompt(self, prompt, clip_model_index: int = 0,):
        device = self._execution_device
        ...
        masked_embeddings = self.text_encoder_3(
            masked_ids.to(device),
            attention_mask=None,
        ).last_hidden_state

        # print('masked_embeddings 0', masked_embeddings.keys())

        # masked_embeddings = masked_embeddings.mean(dim=1)
        masked_embeddings = masked_embeddings[:,0,:]

        print('masked_embeddings 1', masked_embeddings.shape)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        # device
        execution_device = self.text_encoder.device
        
        negation_warmup_start = kwargs.get("negation_warmup_start", 1000)
        negation_warmup_end = kwargs.get("negation_warmup_end", 880)
        ...
        neg2_text_embeddings = self._new_encode_negative_prompt2(negative_prompt2, 256, 1)
        ...
        self.text_encoders_to_cpu()
        self.vae_transformer_to_device(device=execution_device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                prompt_embeds_ = rescaled_text_embeddings #if  (0 <= i <= 4) else prompt_embeds

                prompt_embeds_  = prompt_embeds_.to(prompt_embeds.dtype)
        ...
        # Offload transformer
        self.vae_transformer_to_cpu()
        self.text_encoders_to_device(device='cuda')
