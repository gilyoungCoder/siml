...
SD_FUNCTIONS = {
    ...
    "safree_neg_prompt": SafreeDiffusion3Pipeline,
    # repellency class
    "safree_neg_prompt_rep_time": SaferDiffusion3Pipeline, # main table t \in \{1000, 800\}
}
    ...
    for _iter, data in (dataset.iterrows() if isinstance(dataset, pd.DataFrame) else enumerate(dataset)):
        ...
        for _category in _categories:
            if _category in category_dict.keys():
                category_dict[_category].append(0.0)
                pass
            else:
                category_dict[_category] = [0.0]
