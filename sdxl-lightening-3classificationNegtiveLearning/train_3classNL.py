...
1000-step DDPM noise-injected latent training with Negative Learning (NL).
Uses nn.NLLLoss + uniform bias initialization, and reports NL loss at validation.
"""
...
# --------------------------------------------------------------------------- #
#  Negative Learning Loss (NLLLoss + log(1−p))                                #
# --------------------------------------------------------------------------- #
nll_loss = nn.NLLLoss()
...
def nl_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    """
    NL loss via NLLLoss:
      - sample one complementary label per sample (not the true label)
      - loss = -log(1 - p_comp)
    """
    with torch.no_grad():
        B, C = logits.shape
        # sample comp labels in [0, C) excluding true
        comp = torch.randint(0, C-1, (B,), device=logits.device)
        comp = torch.where(comp >= labels, comp + 1, comp)
    # compute log(1 - p_k)
    log_probs = torch.log(1.0 - F.softmax(logits, dim=-1) + 1e-8)
    # NLLLoss
    return nll_loss(log_probs, comp)


# --------------------------------------------------------------------------- #
#  Initialize classifier to uniform output (softmax ≡ [1/3,1/3,1/3])           #
# --------------------------------------------------------------------------- #
def init_classifier_uniform(m):
    if isinstance(m, nn.Linear):
        # zero weights, zero bias → logits = 0 → softmax = uniform
...
# --------------------------------------------------------------------------- #
def get_args():
    p = argparse.ArgumentParser("3-class nudity classifier (NL only)")
    # data
    ...
    # model / output
    p.add_argument("--output_dir", type=str, default="clf3_nl_out")
    ...
    p.add_argument("--wandb_project", default="nudity3_nl")
...
if __name__ == "__main__":
    ...
    sched = DDPMScheduler.from_pretrained(args.sdxl_base, subfolder="scheduler",
    ...
    ds = ThreeClassDataset(args.not_people_data_path,
    ...
    # Classifier
    clf = load_discriminator(ckpt_path=None, condition=None,
                             eval=False, channel=4, num_classes=3).to(dev)
    # initialize to uniform output
    clf.apply(init_classifier_uniform)

    opt = torch.optim.AdamW(clf.parameters(), lr=args.learning_rate)

    # Prepare with accelerator
    clf, opt, tr_loader, va_loader = acc.prepare(clf, opt, tr_loader, va_loader)
    ...
    while step < max_steps and no_improve < patience:
        for batch in tr_loader:
            ...
            if step % 100 == 0:
                ...
                logger.info(f"[VAL @ step {step}] NL_loss: {avg_nl:.4f}, Acc: {val_acc:.4f}")
