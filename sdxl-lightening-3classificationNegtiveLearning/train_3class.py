...
def nl_loss(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    """
    Negative Learning loss:
      - sample a complementary label per sample (not the true label)
      - L = - ∑_{k} [ȳ_k * log(1 - p_k)]
    """
    probs = F.softmax(logits, dim=-1)        # [B, C]
    B, C = probs.shape

    # 1) complementary label sampling in [0, C-1) excluding true label
    comp = torch.randint(0, C - 1, (B,), device=probs.device)
    # 2) if comp >= true label, shift up by 1
    comp = torch.where(comp >= labels, comp + 1, comp)

    # 3) one-hot encode
    comp_onehot = F.one_hot(comp, num_classes=C).float()  # [B, C]

    # 4) compute NL loss: only complementary positions contribute
    #    -log(1 - p_comp)
    loss_per_sample = -(comp_onehot * torch.log(1.0 - probs + 1e-8)).sum(dim=1)
...
# --------------------------------------------------------------------------- #
def get_args():
    p = argparse.ArgumentParser("3-class nudity classifier (Lightning) - NL")
    # data
