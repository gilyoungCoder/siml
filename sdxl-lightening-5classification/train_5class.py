...
"""
5-class classifier for SDXL-Lightning
(0: non-people (ImageNet) | 1: fully-clothed-everyday |
 2: fully-clothed-swimwear | 3: partial-nude | 4: full-nude)
...
class FiveClassDataset(Dataset):
    """0: non_people (ImageNet)
       1: fully_clothed_everyday
       2: fully_clothed_swimwear
       3: partial_nude
       4: full_nude
    """
    def __init__(self,
                 non_people_dir,
                 fully_clothed_everyday_dir,
                 fully_clothed_swimwear_dir,
                 partial_nude_dir,
                 full_nude_dir,
                 transform=None):
        samples = []
        for root, label in [
            (non_people_dir,                   0),
            (fully_clothed_everyday_dir,       1),
            (fully_clothed_swimwear_dir,       2),
            (partial_nude_dir,                 3),
            (full_nude_dir,                    4),
        ]:
...
def get_args():
    p = argparse.ArgumentParser("5-class nudity classifier")
    # data paths
    p.add_argument("--imagenet_data_path",              required=True)
    ...
    # output / model
    p.add_argument("--output_dir",    default="clf5_out")
    ...
    p.add_argument("--wandb_project",   default="nudity5")
...
def main():
    ...
    clf = load_discriminator(
        ...
        eval=False, channel=4, num_classes=5
    ).to(device)
