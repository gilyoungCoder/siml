...
% CAS diamond (red) — formula only, no threshold annotation
...
% Anchor-inpaint branch (TOP) — equation lives deeper in box now that tag chip is removed
\node[colBG,fill=anchorBG,minimum width=\colWin cm,minimum height=2.5cm,anchor=north]
...
\node[colBG,fill=hybridBG,minimum width=\colWin cm,minimum height=2.5cm,anchor=north]
     (hybBox) at (\Dx, -0.4) {};
...
% merge point pushed RIGHT so safeEps+safeImg clear the Hybrid equation box
\coordinate (mergeNode) at (\Dx + \colW/2 + 0.25, 0.1);
...
% feedback to B loop — update x to match new mergeNode position
\draw[bigarrow,draw=howPurple!70!black,dashed]
  (safeEps.north) -- ++(0,0.5) -| (\Dx + \colW/2 + 0.25, 5.75)
  -- (\Bx - \colW/2 + 0.5, 5.75)
