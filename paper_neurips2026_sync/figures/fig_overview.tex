...
\node[colBG,fill=txtBG,minimum width=\colWin cm,minimum height=3.1cm,anchor=north]
     (txtBox) at (\Ccx, 2.75) {};
\node[anchor=north,font=\scriptsize\bfseries,text=txtAccent] at (\Ccx,2.63) {Text probe};
...
% shorter formula: keep only the key "what is this probe" — max of keyword attention
...
\node[colBG,fill=imgBG,minimum width=\colWin cm,minimum height=3.1cm,anchor=north]
     (imgBox) at (\Ccx, -0.55) {};
\node[anchor=north,font=\scriptsize\bfseries,text=imgAccent] at (\Ccx,-0.67) {Image probe};
